...
function renderTableToHtml(tableLines, precedingTitle = "", hideWrapper = false, hideRemarks = false) {
  ...
  html += `<span class="text-xs sm:text-sm font-extrabold text-slate-300 select-none flex items-start gap-1.5">`;
  html += `<span>📊</span>`;
  ...
  html += `</button>`;
  html += `<button onclick="const container=this.closest('.table-export-wrapper').querySelector('.markdown-table-container'); const isHidden=container.classList.toggle('hidden'); this.querySelector('.toggle-label').textContent=isHidden?'▼ 펼치기':'▲ 접기';" class="text-xs font-bold bg-slate-800/90 hover:bg-slate-700 text-slate-300 border border-slate-700/80 px-2.5 py-1 rounded-lg transition-all flex items-center gap-1 cursor-pointer select-none">`;
