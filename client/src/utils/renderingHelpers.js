...
export function convertMarkdownToHtml(mdText, isMarkdown = false, highlightBold = false, isTutor = false, isExplanation = false) {
  ...
  // Convert markdown list items (*, -, •, or 1., 2.) including multi-level indentation into standard clean lists
  tempText = tempText.replace(/((?:^[ \t]*(?:(?:>|&gt;)\s*)?(?:[-*•▪▫·]|\d+[\.\)](?!\d))\s*.+(?:\n|$))+)/gm, (block) => {
    const lines = block.split('\n');
    let html = '';
    let inOuter = false;
    let inInner = false;
    let currentOuterType = 'ul';
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!isSubItem) {
        ...
        if (!inOuter) {
          currentOuterType = listTag;
          const listStyle = isOrdered ? 'list-style-type: decimal;' : 'list-style-type: disc;';
          html += `<${listTag} style="${listStyle} padding-left: 1.3rem; margin: 0.3rem 0 0.3rem 0;">`;
          inOuter = true;
        } else if (currentOuterType !== listTag) {
          html += `</${currentOuterType}>`;
          currentOuterType = listTag;
          const listStyle = isOrdered ? 'list-style-type: decimal;' : 'list-style-type: disc;';
          html += `<${listTag} style="${listStyle} padding-left: 1.3rem; margin: 0.3rem 0 0.3rem 0;">`;
        }
        ...
        if (isOnlyBlockMath) {
          ...
          html += `<li style="margin-bottom: 0.25rem; line-height: 1.65;">${content}</li>`;
        }
      } else {
        if (!inOuter) {
          currentOuterType = 'ul';
          html += '<ul style="list-style-type: disc; padding-left: 1.3rem; margin: 0.3rem 0 0.3rem 0;">';
          inOuter = true;
        }
        if (!inInner) {
          html += '<ul style="list-style-type: circle; padding-left: 1.2rem; margin: 0.15rem 0;">';
          inInner = true;
        }
        ...
        if (isOnlyBlockMath) {
          ...
          html += `<li style="margin-bottom: 0.2rem; line-height: 1.6; color: #cbd5e1;">${content}</li>`;
        }
      }
    }
    if (inInner) html += '</ul>';
    if (inOuter) html += `</${currentOuterType}>`;
    return html;
  });
