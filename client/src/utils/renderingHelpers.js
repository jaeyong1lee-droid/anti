...
} from '../components/plugins/DynamicWrapperPlugin.js';
...
export function convertMarkdownToHtml(mdText, isMarkdown = false, highlightBold = false, isTutor = false) {
  ...
  // Protect HTML table blocks and pre-rendered AI Tutor Diagram Cards (SVG, TikZ, Mermaid)
  const tableBlocks = [];
  let tempText = mdText || '';
  ...
  // Restore HTML tables and rendered diagram cards
  ...
  // Restore code blocks (which include rendered SVG/TikZ/Mermaid cards)
  ...
  tempText = applyAllDynamicWrappers(tempText);
  return tempText;
}
...
export const cleanAndSanitizeMathText = (rawText) => {
  ...
  // Restore shielded diagram card HTML blocks
  diagramCards.forEach(card => {
    while (cleaned.includes(card.placeholder)) {
