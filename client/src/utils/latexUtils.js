...
1. 🚨 [수식 달러 기호($) 감싸기 필수 및 역슬래시 유출 엄격 금지]: 모든 수학 공식, 변수 기호, 괄호 결합 함수식(예: $K_s$, $e$, $\\sigma$, $\\Delta t$, $S(t)$, $S(\\Delta t)$, $f(x)$ 등)은 단 하나도 빠짐없이 100% $변수명$ 형태의 달러 기호로 감싸서 출력하십시오. 백슬래시(\\) 기호가 달러 기호 바깥에 텍스트로 날것 노출되는 출제 행위를 엄격히 금지합니다. 반드시 $\\gamma_w$ 와 같이 달러 기호로 감싸서 출력하십시오. 보기 문항과 해설(explanation, answer 등)에도 수식을 적극적으로 활용하되 반드시 기호로 감싸야 합니다.
...
13. 🚨 [마크다운 리스트 수칙]: JSON 응답 내에서 항목을 나열하기 위해 리스트 기호(* 또는 -)를 사용할 때는 반드시 기호 뒤에 스페이스(공백)를 한 칸 띄우고 텍스트를 작성하십시오. (예: "* k: 투수계수" (O) / "*k: 투수계수" (X)). 
...
1. 🚨 [수식 달러 기호($) 감싸기 필수 및 역슬래시 유출 엄격 금지]: 모든 수학 공식, 변수 기호, 괄호 결합 함수식(예: $K_s$, $e$, $\\sigma$, $\\Delta t$, $S(t)$, $S(\\Delta t)$, $f(x)$ 등)은 단 하나도 빠짐없이 100% $변수명$ 형태의 달러 기호로 감싸서 출력하십시오. 백슬래시(\\) 기호가 달러 기호 바깥에 텍스트로 날것 노출되는 출제 행위를 엄격히 금지합니다. 반드시 $\\gamma_w$ 와 같이 달러 기호로 감싸서 출력하십시오. 답변에도 수식을 적극적으로 활용하되 반드시 기호로 감싸야 합니다.
...
12. 🚨 [마크다운 리스트 수칙]: 항목을 나열하기 위해 리스트 기호(* 또는 -)를 사용할 때는 반드시 기호 뒤에 스페이스(공백)를 한 칸 띄우고 텍스트를 작성하십시오. (예: "* k: 투수계수" (O) / "*k: 투수계수" (X)). 
