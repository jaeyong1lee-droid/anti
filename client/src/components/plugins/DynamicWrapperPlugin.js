          ...
          if (isNoneText || /해당\s*검색.*없습니다/i.test(detailContent)) {
            ...
            if (isPipingTopic) {
              detailContent = isKds
                ...
                : `${titleSummary}: Wikipedia Soil Mechanics (Seepage & Piping Mechanism) - Piping occurs when the upward seepage force exceeds the submerged unit weight of the soil, causing quick condition and backward erosion. The critical hydraulic gradient ic is mathematically expressed as ic = (Gs - 1)/(1 + e) = γ'/γw. Countermeasures focus on increasing the seepage path length L via cutoff walls or reducing the total head differential Δh to lower the exit gradient iexit.<br/><br/>파이핑(Piping) 현상은 상향 침투수력이 흙의 수중 단위중량을 초과할 때 발생하며, 보일링(Quick condition) 및 역행성 침식을 유발함. 임계동수경사 ic는 수학적으로 ic = (Gs - 1)/(1 + e) = γ'/γw 로 표현됨. 방지 대책은 차수벽(Cutoff wall)을 통해 침수 유로 길이 L을 연장하거나, 총 유효 수두차 Δh를 줄여 유출 동수경사 iexit를 낮추는 것에 집중됨.`;
            } else {
              detailContent = isKds
