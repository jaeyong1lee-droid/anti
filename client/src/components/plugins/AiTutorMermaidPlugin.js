...
\n<div class="my-6 w-full max-w-4xl mx-auto bg-[#0b0f19] rounded-2xl p-6 border border-slate-800 shadow-2xl select-text font-sans">
  <div class="flex items-center justify-between border-b border-slate-800/80 pb-4">
    <div class="flex items-center gap-2">
      ...
      <span class="text-[10px] font-extrabold bg-emerald-950/80 text-emerald-400 border border-emerald-500/30 px-3 py-1 rounded-full uppercase tracking-wider">⚡ 머메이드 2D 벡터 순서도</span>
    </div>
...
export function renderAiTutorMermaid(text) {
  ...
  if (text.includes('diagram-card-content') || text.includes('Mermaid Realtime Vector') || text.includes('___DIAGRAM_CARD_')) return text;
