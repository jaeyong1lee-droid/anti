...
export function sanitizeAiTutorKatexText(rawText) {
  ...
  // 1. Shield Hex Color Codes (#e7f5ff, #868e96, etc.) and Markdown Headings (###) from KaTeX
  text = text.replace(/(fill|stroke|color):\s*#([a-fA-F0-9]{3,8})/gi, '$1: __HEXCOLOR_$2__');
  text = text.replace(/(fill|stroke|color)=['"]*#([a-fA-F0-9]{3,8})['"]*/gi, '$1="__HEXCOLOR_$2__"');

  // 2. Normalize single-quoted double quotes generated by LLM (e.g. fill=''#868e96'')
