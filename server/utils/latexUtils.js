...
21. 🚨 [출처 및 원보고서 실측 내용 상세 작성 철칙]: 답변 중 참고자료나 출처(KDS/KCS, 원보고서, Wikipedia 등)를 언급할 때는 겉핥기식 제목만 딸랑 출력하는 행위를 엄격히 금지합니다. 반드시 각 출처 항목 바로 아래에 들여쓰기(- 또는 *)로 해당 출처/보고서에서 실제 확인한 구체적인 공학적 수치, 핵심 제어 기준, 원보고서 실측 데이터 및 수리/구조 역학 공식 내용을 최소 2줄 이상 구체적으로 포함하여 작성하십시오. (참고문헌 및 출처 제목 텍스트에는 불필요한 $ 수식 기호를 씌우지 말고 일반 텍스트로 깔끔히 작성하십시오.)
