...
7. 분수(\\\\frac), 거듭제곱근(\\\\sqrt) 등이 포함된 복잡한 수식은 일반 텍스트 문단과 섞이지 않도록 반드시 줄바꿈(엔터)을 하여 수식만 단독으로 분리해서 작성하되, ⚠️절대로 한글 설명이나 일반 문장 전체를 달러 기호로 통째로 감싸지 마십시오. 한글 문장과 수학 기호는 철저히 분리하여, 수학 기호에만 달러 기호를 적용하십시오.
