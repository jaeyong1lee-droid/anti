...
1. 🚨 [수식 달러 기호($) 감싸기 필수 및 역슬래시 유출 엄격 금지]: 모든 수학 공식, 변수 기호, 괄호 결합 함수식(예: $, $, $\\\\sigma$, $\\\\Delta t$, (t)$, (\\\\Delta t)$, (x)$ 등)은 단 하나도 빠짐없이 100% $ 형태의 달러 기호로 감싸서 출력하십시오. 날것의 텍스트 표기(예: \\\\gamma_w)나 마크다운 백틱() 사용은 엄격히 금지합니다. 반드시 $\\\\gamma_w$ 와 같이 감싸십시오. 보기 문항과 해설(explanation, answer 등)에도 수식을 적극적으로 활용하되 반드시 기호로 감싸야 합니다.
