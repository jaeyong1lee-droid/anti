...
5. 외부 공백 필수 조건: $ 기호의 앞과 뒤가 한글, 숫자, 문장 부호와 맞닿을 경우 반드시 앞뒤로 '한 칸의 공백(스페이스)'을 명시적으로 두어 격리하십시오. 한국어 조사('가', '는', '입니다' 등)와 결합할 때도 예외 없이 한 칸 띄우고 조사를 작성하십시오. 단, 괄호 '()'는 예외입니다. 괄호로 감싸인 수식이나 변수(예: S(\\\\Delta t))는 괄호를 수식의 일부로 취급하여 통째로 $ 기호 내부에 온전히 포함시켜야 하며 절대 수식을 쪼개지 마십시오. (예: $S(\\\\Delta t)$ (O) / $S$ ( $\\\\Delta t$ ) (X))
...
4. 외부 공백 필수 조건: $ 기호의 앞과 뒤가 한글, 숫자, 문장 부호와 맞닿을 경우 반드시 앞뒤로 '한 칸의 공백(스페이스)'을 명시적으로 두어 격리하십시오. 한국어 조사('가', '는', '입니다' 등)와 결합할 때도 예외 없이 한 칸 띄우고 조사를 작성하십시오. 단, 괄호 '()'는 예외입니다. 괄호로 감싸인 수식이나 변수(예: S(\\Delta t))는 괄호를 수식의 일부로 취급하여 통째로 $ 기호 내부에 온전히 포함시켜야 하며 절대 수식을 쪼개지 마십시오. (예: $S(\\Delta t)$ (O) / $S$ ( $\\Delta t$ ) (X))
