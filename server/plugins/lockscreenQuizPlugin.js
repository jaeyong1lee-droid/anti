...
특히 제시된 [출제 지침 기준]을 최우선적으로 준수하여 출제해야 합니다. 단순 이론/개념 설명 문제는 배제하고 공식, 설계 기준, 구체적 수치/숫자값 중심의 깊이 있는 학술적 응용 문제를 출제하십시오.
...
[출제 요구사항]:
1. **🚨 [최우선 출제 지침 기준 준수]**:
   - 제시된 **[출제 지침 기준 (Lockscreen Generation Standards)]**에 적혀 있는 지침 사항들을 **최우선적으로, 타협 없이 엄격히 준수**하여 출제하십시오.
   - 단순 서술형 개념 설명이나 일반적인 이론 설명은 **절대 배제**하십시오.
   - 반드시 **공식(수식 매칭, 변수 정의, 비례 관계 등), 설계 및 시공 기준(KDS 등), 구체적인 정량적 수치/숫자값(상/하한값, 단위, 상수 등)**만 질문 대상으로 삼으십시오.
   - 토픽 본문 단어를 단순 빈칸으로 메우거나 그대로 베끼는 **1차원적이고 단순한 출제는 엄격히 금지**합니다. 거동 원리/메커니즘, 공식 유도 과정 및 가정 조건 등을 종합적으로 반영하여 학술적 깊이가 있는 응용형 문제를 구성하십시오.
