...
[의미 및 맥락 중심의 동적 채점 프레임워크 (Contextual & Semantic Grading Framework)]:
어휘의 단순 일치(Literal Matching)나 키워드의 단순 존재 여부에 의존하는 기계적 평가를 원천 배제하고, 사용자의 답안이 지닌 '공학적 본질'을 모범 답안과 유기적으로 비교·평정하기 위해 다음 채점 프로세스를 엄격히 준수하십시오.

1단계: 질문 지문 및 표(Table) 구조의 맥락 해석
- 질문(question)의 내용 및 표의 행/열 헤더 맥락을 최우선으로 종합하여 분석하십시오.
- 만약 질문이나 표 구조가 '해석 기준 응력'이나 '강도 정수 적용'과 같이 명확한 기호 또는 매개변수 명칭을 요구하고 있으나, DB에 저장된 모범 답안(correctAnswer)이 '간극수압을 차감한 유효 접촉 응력'이나 '단기 안정성 검토에 활용'과 같은 장황한 개념 정의/용도 설명으로 등록되어 있는 경우라 하더라도, 사용자가 질문과 표 헤더에 완벽히 부합하는 올바른 기호나 매개변수 명칭을 적었다면 이는 질문 맥락상 최선의 정답(10점 만점)으로 전적으로 인정해야 합니다. 반대의 경우(모범 답안이 기호인데 사용자가 개념 서술형으로 쓴 경우)도 마찬가지로 만점으로 인정합니다.

2단계: 공학적 기호/약어와 개념적 서술의 등가성 인정 (Engineering Equivalence)
- 지반공학적 기호, 수식, 약어(예: $\\sigma_1, \\sigma_3$, $\\sigma_1', \\sigma_3'$, $c, \\phi$, $c_u, \\phi_u$, $c', \\phi'$, UU, CU, CD 등)와 그 명칭 및 개념적 설명(예: 주응력, 유효응력, 점착력, 내부마찰각, 비배수/배수 조건 등)은 상호 완벽한 동의어로 취급하십시오.
- 모범 답안이 장황한 서술형 문장일지라도 사용자가 이를 상징하는 올바른 기호나 공식 변수명을 작성했다면 절대 감점하지 마십시오.
...
- 10점 (만점): 사용자 답안이 질문에 대한 정확한 공학적 답변이며, 모범 답안과 의미적으로 동등한 개념을 전달하는 경우. (표현 양식, 기호 사용, 요약 수준의 차이로 인한 감점 절대 금지)
