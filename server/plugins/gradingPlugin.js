...
export let gradingStandardsList = [
  ...
  },
  {
    ...
    "content": "[🚨 물리적/수학적 등가 치환 인정 및 한계선 규칙 (Physical/Mathematical Equivalent Substitution & Boundary Rules) - 극도로 중요!]:\n공식의 특정 변수를 지반공학 정의상 물리적·수학적으로 완벽하고 정확하게 부합하는 단일 물리량 명칭, 표준 합성 기호 또는 대입 관계식으로 치환하여 작성한 경우에 한해서만 정답으로 인정하십시오.\n\n엄격한 한계선 및 오답 판정 기준:\n1. 정의 및 의미의 100% 동등성: 치환된 용어가 원래 공식의 물리적 의미를 완벽하게 대체해야 합니다. (예: 유효응력 공식 σ' = σ - u에서 간극수압 u를 '수압' 또는 '정수압'으로 치환하는 것은 허용되나, 전혀 다른 개념인 '침투수력', '수위' 또는 단순히 모호한 '압력'으로 치환한 경우에는 명백한 오답(0점) 처리)\n2. 차원(Dimension) 및 단위의 일치: 치환된 결과물은 원래 공식 해당 부분의 물리적 차원(단위)과 반드시 일치해야 합니다. (예: 침투압 공식 ps = i * z * γ_w에서 z * γ_w를 '수압'으로 치환하여 '동수경사 * 수압'으로 적는 것은 차원이 압력으로 일치하므로 허용되나, 힘 단위인 '수력'이나 무차원인 '수두'로 치환하는 것은 차원 오류로 오답 처리)\n3. 부호 및 연산자 정밀성: 치환이 이루어진 수식에서도 부호(+, -) 및 사칙연산의 관계가 완벽히 보존되어야 합니다. (예: 유효응력 공식에서 σ - 수압은 정답이지만, σ + 수압이나 σ - 수압 + 침투압 등 연산 기호나 방향이 잘못된 경우는 치명적 오개념으로 오답 처리)\n4. 과도한 동조(Sycophancy) 금지: 사용자가 쓴 단어가 모범 답안의 단어와 유사하다는 이유만으로, 물리적/수학적 정의가 틀렸음에도 긍정적으로 해석하여 부분점수를 주는 과도한 유연 채점을 절대 금지합니다. 수식 판정은 언제나 물리학 및 지반공학 법칙에 근거해 차원과 부호가 완벽해야 합니다."
  }
];
