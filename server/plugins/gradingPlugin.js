...
🚨 **[경고 - 표 빈칸 행/열 및 아스키 플로우차트 단계/분기명 매핑 교차 검증 및 자가 치유 철칙 - 극도로 중요!]**:
- 표 채우기 및 플로우차트 문항 채점 시, DB에 기록된 모범 답안(correctAnswer)이 오매핑(예: 행 제목이 '산정 방법'인데 모범 답안에 '응용 범위'가 들어있거나, 순서도 분기 조건 상자 (E) 위치에 다른 단계의 장문 설명이 들어있는 오매핑 현상)되어 있는지 채점 전에 반드시 공학적으로 상호 교차 검증하십시오!
- 만약 모범 답안(correctAnswer)의 내용이 해당 셀의 위치(행 제목: '${rowHeader || '미지정'}', 열 제목: '${colHeader || '미지정'}') 또는 순서도 상자 맥락(분기 조건/단계명)과 부합하지 않는 매핑 교차 오류인 경우:
  1) DB의 잘못된 모범 답안(correctAnswer)을 즉시 출제 오매핑 오류로 완전 무시하십시오.
  2) 오직 표의 위치/순서도 흐름 상자의 '진짜 올바른 공학적 정답(예: 만족 시, 포화단계, 등방압밀 등)'을 채점관의 전문 지식으로 직접 도출하십시오.
  3) 사용자의 제출 답안('${userAnswer}')이 새로 도출한 그 진짜 올바른 공학적 정답에 부합하면 반드시 10점 만점을 부여하십시오! (절대 잘못 매핑된 DB 답안과 대조하여 감점하지 마십시오!)
  4) suggestedModelAnswer에는 오직 해당 위치에 부합하는 진짜 올바른 전공 정답만을 반환하십시오.
