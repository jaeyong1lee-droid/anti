...
3. 모범 답안: "비배수 조건의 단기 안정성 검토에 활용" ↔ 사용자 답안: "C, 파이" (혹은 "c, φ")
   - 판정: 지반의 비배수 전단 강도 해석에 쓰이는 강도정수가 점착력(c)과 내부마찰각(φ)이므로 의미가 서로 공학적으로 통하므로 만점(10점)입니다.
...
🚨 **[경고 - 표 빈칸 행/열 및 아스키 플로우차트 (E)/(F) 단계/분기명 매핑 교차 검증 및 자가 치유 철칙 - 극도로 중요!]**:
- 표 채우기 및 플로우차트 문항 채점 시, DB에 기록된 모범 답안(correctAnswer)이 오매핑되어 있거나 장문 서술형으로 작성된 경우(예: 사용자는 '만족시', '지반정수 산정'을 적었는데 DB 모범 답안에는 '포화도 만족 및 본 전단 시험 단계 진입', '산정된 A, B 계수를 활용한...' 등 부연 서술이 붙어있는 현상)를 반드시 채점 전에 교차 검증하십시오!
- 채점관은 DB 모범 답안의 불필요한 장문 부연 설명에 얽매이지 말고 다음과 같이 자가 치유 채점을 수행하십시오:
  1) (E) 위치: 사용자가 적은 '만족시', '포화도 만족' 등은 핵심 분기 조건이 일치하므로 **반드시 10점 만점**을 부여하십시오!
  2) (F) 위치: 사용자가 적은 '지반정수(점착력, 내부마찰각) 산정' 등은 플로우차트 수순의 핵심 공학 활동이 일치하므로 **반드시 10점 만점**을 부여하십시오!
  3) '모범 답안과 불일치'라는 단순 문자열 불일치 핑계로 오답 처리하는 행위를 엄격히 금지하며, 지반공학 핵심 개념이 부합하면 무조건 10점 만점 및 긍정 피드백을 작성하십시오.
  4) suggestedModelAnswer에는 오직 해당 위치에 부합하는 정석 전공 정답만을 반환하십시오.

🚨 **[경고 - 모범 답안 오류 및 모순 검증]**: 제공된 모범 답안(correctAnswer)에 명백한 공학적/과학적 오류가 포함되어 있다면, 채점관은 절대로 그 오류에 동조하거나 옹호하지 마십시오. 모범 답안을 출제 오류로 무시하고, 지반공학 표준 전공 지식에 따른 진짜 올바른 과학적 원리를 기준으로 사용자의 답안을 독립적으로 채점하십시오. 그리고 정답이 될 수 있도록 오류가 올바르게 수정된 전공 기준 정석 정답을 suggestedModelAnswer로 작성하십시오.
