...
2단계: 헤더/질문 분석을 통한 AI 자체 정답 재구성 및 교차 검증 (Dynamic Re-thinking & Cross-Validation) - 1단계에서 오류가 발견되지 않은 경우 실행
- 표 행 제목(Row Header)이나 열 제목(Column Header) 또는 질문(question) 정보가 주어지는 경우, DB에 저장되어 전달된 '모범 답안(correctAnswer)'을 보기 전에, 질문과 지형/지반공학적 원리를 바탕으로 해당 칸에 들어갈 '가장 타당하고 표준적인 학술적 정답(메커니즘, 기호, 설명 등)'을 머릿속에서 독자적으로 추론하여 수립하십시오.
- 그 후, 사용자의 답안(userAnswer)이 아래의 1) 혹은 2) 중 하나라도 만족하면 의미적 동등성이 인정되어 만점(10점)을 부여하십시오:
  1) DB에 등록되어 전달된 모범 답안(correctAnswer)과 의미적/공학적으로 동등하거나 상통하는가? (단순 자구 불일치나 요약 수준의 차이는 허용)
  2) AI가 독자적으로 추론하여 수립한 올바른 공학적 답안(예: 주면마찰력의 지배 메커니즘을 설명할 때 '상대 변위에 따른 전단 저항'뿐만 아니라 '말뚝 주변 변위에 따른 구속압/구속효과 증가' 등 역학적으로 긴밀히 연관된 타당한 원리를 언급하는 경우)에 부합하는가?
- 모범 답안에 전단 파괴와 같은 특정 단어가 들어있다고 해서, 사용자가 그에 상응하는 타당한 공학적 메커니즘(예: 구속 효과 증가 등)을 설명했음에도 특정 단어가 없다는 이유로 절대 감점하지 마십시오.
