...
🚨 **[경고 - 수치 계산 문항 원보고서/해설 정밀 수치 검증 수칙 (Exact Numeric Tolerance Rule)]**:
- 수치 계산 문항(행 제목: '${rowHeader}', 열 제목: '${colHeader}') 채점 시, 채점관은 문제 지문('${question}'), 행 제목('${rowHeader}'), 전체 해설('${explanation}')에 명시된 원보고서의 '실제 정밀 계산 수치값(Real Calculated Value)'을 직접 도출하거나 해설 수치를 대조하십시오.
- **사용자의 입력 수치('${userAnswer}')가 원보고서의 실제 정밀 계산 수치값과 일치하거나 부합하는지 엄격히 검증**하십시오:
  1) 사용자의 답안('${userAnswer}')이 원보고서 정답 수치(예: 632 또는 417 등)와 ±5% 이내(또는 소수점 반올림 오차 수준)로 거의 일치하는 경우 -> **10점 만점 정답** 및 이유 서술.
  2) 사용자의 답안('${userAnswer}')이 원보고서 정답 수치와 ±5% ~ ±15% 범위의 부분 수치인 경우 -> **6.3점~8점 부분 점수** 및 이유 서술.
  3) 사용자의 답안('${userAnswer}')이 원보고서의 정답 수치와 15% 이상 크게 다르거나 틀린 수치인 경우 -> **0점~5점 이하 오답 처리** 및 오답 사유 서술.
- **[원보고서 수치 환각(Hallucination) 절대 금지]**: 사용자가 틀린 수치(예: '700')를 적었을 때, 그 사용자의 수치('700')를 그대로 모방하여 suggestedModelAnswer나 해설에 "허용지지력 q_all = 700 이 도출됩니다"라고 받아 적는 행위를 100% 금지합니다! suggestedModelAnswer에는 오직 원보고서/공학 공식에 의한 진짜 수치 정답(예: 약 632 또는 417 kN/m²)과 그 도출 과정을 정확히 작성하십시오.
