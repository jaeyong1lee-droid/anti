...
🚨 **[경고 - 표 빈칸 행/열 매핑 교차 검증 및 행/열 제목 미일치 오매핑 자가 치유 철칙 - 극도로 중요!]**:
- 표 채우기 문항 채점 시, DB에 기록된 모범 답안(correctAnswer)이 오매핑(예: 행 제목이 '${rowHeader || '미지정'}'인데 표의 다른 행인 '대표성' 등 엉뚱한 행의 답안이 적혀 있는 현상, (B)칸(INPUT_2)에 (C)/(D) 답안이 적힌 오류, 또는 (B)와 (D) 답안이 동일한 매핑 오류)되어 있는지 채점 전에 반드시 공학적으로 상호 교차 검증하십시오!
- 만약 모범 답안(correctAnswer)이 표의 행 제목('${rowHeader || '미지정'}') 및 열 제목('${colHeader || '미지정'}')의 교차점에 부합하지 않는 매핑 오류(Row/Column Header Mismatch)인 경우:
  1) DB의 잘못 오매핑된 모범 답안(correctAnswer)을 즉시 출제 매핑 오류로 완전 무시하십시오.
  2) 오직 표의 행 제목('${rowHeader || '미지정'}')과 열 제목('${colHeader || '미지정'}')이 만나는 해당 격자 칸의 '진짜 올바른 공학적 정답'을 채점관의 전문 지식 및 해설(explanation)을 참조하여 직접 도출하십시오. (예: 행 제목이 '적용 대상'이고 열 제목이 '실내 투수시험'이면 '채취 시료' 또는 '실내 시험용 시료' 등이 올바른 정답이며, '대표성' 행의 정답인 '낮음'을 적용해서는 안 됨)
  3) 사용자의 제출 답안('${userAnswer}')이 새로 도출한 그 진짜 올바른 공학적 정답에 부합하면 반드시 10점 만점을 부여하십시오! (절대 오매핑된 DB 답안과 대조하여 오답/감점 처리하지 마십시오!)
