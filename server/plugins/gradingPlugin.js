...
1. 수식, 공식, 수학적 관계식 및 계산 결과(숫자) 답안 (엄격 판정):
   - 공식, 수식, 수학적 관계식 및 계산 결과 수치는 자구와 구성 요소가 완벽히 정확해야 합니다.
   - 공식의 구성 변수(예: Ka, H 등), 상수, 부호(+, -), 지수(승수) 중 하나라도 누락되거나 다르게 작성한 경우(예: 'pa = 0.65 * 감마 * h' vs 'pa = 0.65 * Ka * 감마 * H')는 치명적인 오개념(오답)으로 판정하여 10점 만점 기준 반드시 0점(또는 최대 1점)으로 처리하십시오. 의미가 비슷하다거나 유도 과정이 대략 유사하다는 이유로 만점이나 높은 부분점수를 부여하는 것을 절대 금지합니다.
