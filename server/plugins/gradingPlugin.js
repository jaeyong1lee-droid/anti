...
1. 수식, 공식, 수학적 관계식 및 계산 결과(숫자) 답안 (엄격 판정):
   ...
   - 답안에 나열된 여러 공식 중 **단 하나라도** 구성 변수(예: Ka, H 등), 상수, 부호(+, -), 지수(승수) 중 일부가 누락되거나 다르게 작성된 경우(예: 'pa = 0.65 * 감마 * h' 처럼 $K_a$가 누락된 경우)는 **답안 전체를 치명적인 오개념(오답)으로 판정**해야 합니다.
   - 이 경우, 다른 공식들이나 한글 설명이 아무리 완벽하더라도 부분점수를 절대 많이 주어서는 안 되며, **반드시 오답 판정(isCorrect: false, score: 10점 만점 기준 0점 또는 최대 1점)**으로 처리하십시오. 소폭 감점 후 '정답 인정(isCorrect: true)'이나 2점 이상의 부분점수를 부여하는 것을 **극도로 엄격히 금지**합니다.
