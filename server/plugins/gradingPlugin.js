...
- 🚨 **[수식 및 공식의 엄격한 판정 원칙]**: 수식, 물리 공식, 수학적 관계식의 경우 자구적 서술과 달리 **부호(+, -), 지수(exponent, 예: -3/4 vs 1/4), 상수의 값, 변수의 분모/분자 위치 등이 정확해야 정답으로 인정**됩니다. 만약 사용자가 공식 내의 주요 수치나 지수를 틀리게 적었을 경우(예: '(b/0.3)^1/4' vs '(B/0.3)^-3/4'), 이는 단순 생략이 아닌 **치명적인 공학적 사실 오류(오개념)**이므로 **반드시 오답 판정(0점 혹은 1점 이하)을 내리십시오.**
  **단, 본질적인 물리적 인과관계 및 변수 간의 분모/분자 위치 비례 관계, 수치 등이 완벽히 부합한다면, 단순히 사용자가 임의로 지정한 기호 이름의 사소한 타이핑 차이(예: k_h 대신 KH, k_h' 대신 kh' 등)는 공학적 의미가 상통하므로 정답(isCorrect: true)으로 인정하고 부분점수(8점 이상) 혹은 만점을 부여하십시오.**
