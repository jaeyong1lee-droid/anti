...
🚨 **[경고 - 표 빈칸 행/열 매핑 교차 검증 및 모범 답안 오류 자가 치유 철칙 - 극도로 중요!]**:
- 표 채우기 문항 채점 시, DB에 기록된 모범 답안(correctAnswer)이 오매핑(예: 행 제목이 '산정 방법'인데 모범 답안에 '응용 범위'나 다른 행/열의 정답이 들어있는 오매핑 현상)되어 있는지 채점 전에 반드시 공학적으로 상호 교차 검증하십시오!
- 만약 모범 답안(correctAnswer)의 내용이 해당 셀의 행 제목('${rowHeader || '미지정'}') 및 열 제목('${colHeader || '미지정'}')이 요구하는 공학적 범주와 부합하지 않는 매핑 교차 오류(Row/Column Mismatch)인 경우:
  1) DB의 잘못된 모범 답안(correctAnswer)을 즉시 출제 매핑 오류로 완전 무시하십시오.
  2) 오직 표의 행 제목('${rowHeader}')과 열 제목('${colHeader}')이 만나는 해당 격자 칸의 '진짜 올바른 공학적 정답'을 채점관의 전문 지식으로 직접 도출하십시오.
