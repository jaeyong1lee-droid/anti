...
export let standardsList = [
  ...
  {
    "id": "ascii_art",
    "title": "아스키 아트 기하학적 정밀화 기준",
    "content": "\n[📐 아스키 아트(ASCII Art) 기하학적 정밀화 기준 — 극도로 중요]:\n1. **옹벽 및 흙 쐐기(Wedge)의 기하학적 무결성**:\n   - 옹벽을 아스키 아트로 묘사할 때, 옹벽이 왼쪽에 서 있고 오른쪽에 흙이 있다면 옹벽은 반드시 수직 기호(예: `|`)로 정렬하십시오.\n   - 흙 쐐기(파괴 쐐기)의 상단 지표면 경사($\\beta$)는 옹벽 상단에서 시작하여 자연스럽게 우상향 사선(예: `/`) 또는 수평선(예: `_`)으로 옹벽과 연결되도록 표현하고, 지표면이 옹벽 좌측이나 공중에 붕 떠 있지 않게 그리십시오.\n   - 파괴면(Failure plane, $R$)은 옹벽 최하단(Heel)에서 출발하여 우상향 사선(예: `/` 또는 `\\`의 역방향 배치)으로 뻗어 나가 지표면과 맞닿는 **직각삼각형에 가까운 형태(쐐기형)**로 그리십시오. 절대 대칭형 삼각형(`/`) 형태로 흙 쐐기를 그리지 마십시오.\n   - 토압 화살표($P$)는 옹벽 배면(우측)에 작용하도록 표시하십시오.\n- 아스키 아트 예시 (옹벽 배면 토압 및 쐐기 구조):\n```\n       지표면\n         / \n  옹벽  / [흙 쐐기]\n   |  /   \n   | /   파괴면 (R)\n   |/_________\n```\n"
  }
];
