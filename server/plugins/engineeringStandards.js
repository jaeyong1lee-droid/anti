export let standardsList = [
  {
    ...
    "content": "\n[🚨 1단계: 출처 사전 학습 및 출처 기반 질문/답변 연계 문제 출제/답변 철칙 — 극도로 중요!]:\n- AI는 문제를 출제하거나 해설, 모범답안 및 AI 튜터 답변을 작성할 때, **가장 첫번째 단계(1단계: 사전 지침 및 출처 학습 단계)에서 출처 보고서 및 관련 국가기준/학술문헌(KDS/KCS, 원보고서 본문, Wikipedia Soil Mechanics)의 원문 내용과 지침을 반드시 직접 사전에 깊이 읽고 공부(학습)**해야 합니다.\n- 하드코딩되거나 무관한 찌꺼기 더미 문구(K0, Kirsch, Terzaghi 등)를 절대 사용하지 말고, 이렇게 사전 공부한 출처 문헌의 핵심 공학 현상 및 정론 지침(예: 약액주입 시 용탈, 클로징, 지하수 알칼리성/pH 변화 및 생태계 영향 등 해당 토픽의 실제 공학 메커니즘)에 맞춰 직접 질문(Question)을 설계하고 모범 답안과 100% 매핑하여 출제하십시오.\n- 수험생이 화면의 보고서 항목('')을 확인했을 때 문제의 질문 및 모범답안이 출처의 공학적 지침과 100% 완벽하게 부합하며 깊이 있게 공부할 수 있도록 구성하십시오.\n"
  },
  {
    "id": "stress",
    "title": "MIT 응력 표현 기준",
  ...
  },
  {
    ...
    "title": "핸드폰 세로보기 마크다운 표 렌더링 및 조작 5대 절대 지침",
    "content": "\n[📱 핸드폰 세로보기 마크다운 표 렌더링 및 조작 5대 절대 지침 — 철칙 준수]:\n1. **2열 헤더 더블클릭 2열 너비 맞춤**: 2열 열헤더 우측(또는 셀)을 더블클릭하면 1열을 제외한 나머지 열(2열부터 마지막 열까지)의 너비 간격이 2열 너비 기준으로 동일하게 맞춰져야 합니다.\n2. **열 최소 너비 7글자 보장**: 각 열의 최소 너비는 7글자(약 84px / 7ch) 이하로 작아질 수 없으며, 표 너비가 핸드폰 화면을 넘어가면 수평 스크롤바가 작동해야 합니다.\n3. **2~3개 열 한 화면 맞춤 표출**: 1열 포함 2개 열 또는 3개 열 표는 처음 렌더링 시 한 화면 가로 너비(100%)에 칼같이 맞춰서 보여야 합니다.\n4. **2개 열 100% 가로 폭 유동 연동**: 1열 포함 2개 열 표인 경우 무조건 한 화면 너비(100%) 내에 포함되어야 하며, 1열 헤더를 잡고 움직이더라도 표 전체 너비(100%) 안에서 1열과 2열의 폭이 유동 연동 조정되어 스크롤바가 생기지 않아야 합니다.\n5. **3개 열 이상 각 열 독립 조절 및 전체 표 크기 확장**: 3개 열 이상(또는 4개 열 표)에서 2열이나 특정 열을 잡고 늘리면 1열, 3열, 4열 등 나머지 다른 열의 너비는 절대 줄어들지 않고 고정되어야 하며, 변경된 너비만큼 전체 표 크기(가로 스크롤)가 늘어나야 합니다.\n"
  }
];
