...
1. 1번 문항 (첨부 이미지의 물음과 본문 HTML의 답변을 분석한 표채우기 질문) - type: "주관식 (표채우기)"
   - [우선 수행 사항 - Chain of Thought]: 최종 표를 작성하기 전에 반드시 다음 2단계를 거치십시오.
     1) 'step1_extract': [필독] 반드시 첨부된 **이미지(문제 스크린샷)** 안에 적혀있는 문제 지문 텍스트를 최우선으로 눈으로 직접 읽어내십시오! 제공된 <solution_reference> 태그 안의 HTML 텍스트는 오직 '정답(answers)'을 찾을 때만 참고해야 하며, 문제 지문을 여기서 추출하거나 요약하면 절대 안 됩니다. 🚨 **중요: 해설 본문에 일부 항목의 정답이 누락되어 있더라도, 지문에서 요구하는 항목(예: A, B, C점)은 단 하나도 빼먹지 말고 100% 똑같이 추출해야 합니다! (정답이 없으면 "누락됨"으로 기재할 것)**
     2) 'step2_plan': 추출된 원문 중 쉼표(,)나 '및' 등으로 여러 개를 묻는 복합 항목이 있다면, 최종적으로 수치를 계산해야 하는 독립된 단위 개수만큼 쪼개어(분할하여) 최종 항목 배열을 만드십시오.
   - 표 구조는 반드시 **headers: ["구하는 항목", "계산 결과 및 답안"]** 의 2열 헤더 규격으로 구성하십시오.
   - 🚨 **[절대 지침 - 창작 금지 및 1:1 추출]**: Q1은 오직 "수치 계산용 표"입니다! AI 임의로 이론적인 "특성 비교표"나 "공법 비교표"를 절대 만들지 마십시오. 실무 설계 대책이나 이론적 검토 항목을 창작하는 것을 엄격히 금지합니다.
   - [입력칸(INPUT) 동적 분할 철칙]: 'rows'의 첫 번째 열(구하는 항목)은 반드시 앞서 작성한 'step2_plan'의 항목 배열과 1:1로 정확하게 일치해야 합니다. 각 분할된 항목마다 별도의 \`[INPUT_N]\`이 배정되도록 동적으로 분석하십시오.
   - 🚨 **[환각/창작 절대 금지]**: 'rows'의 첫 번째 열(구하는 항목)에는 원문 텍스트 그대로만 기재하십시오. AI 임의로 지문에 없는 "양압력수두", "한계동수경사" 등의 전문 용어를 지어내거나 의역해서는 안 됩니다. (예: 지문에 "출구까지 동수경사"라고 적혀있으면 그대로 기재). "(A)"나 "(1)" 기호만 쓰지도 마십시오.
   - answers 객체에는 각 INPUT_N마다 대응되는 계산 정답 풀이 과정과 최종 수치를 기재하십시오.
...
[
  {
    ...
    "ocr_text": "이미지에서 눈으로 읽어낸 문제 지문 원문 전체 텍스트",
    "step1_extract": ["(1) 지문 원문 그대로 추출된 첫번째 요구사항", "(2) 복합 조건이 포함된 두번째 원문 그대로 추출"],
    "step2_plan": ["(1) 개별 분할 항목 1", "(2) 기계적으로 분할된 세부 항목 A", "(2) 기계적으로 분할된 세부 항목 B"],
    "tableData": {
      "headers": ["구하는 항목", "계산 결과 및 답안"],
      "rows": [
        ["(1) 개별 분할 항목 1", "[INPUT_1]"],
        ["(2) 기계적으로 분할된 세부 항목 A", "[INPUT_2]"],
        ["(2) 기계적으로 분할된 세부 항목 B", "[INPUT_3]"],
        ["(3) 기타 세부 항목", "[INPUT_4]"]
      ]
    },
    "answers": {
      "INPUT_1": "정답 풀이 1",
      "INPUT_2": "세부 항목 A에 대한 정답 풀이",
      "INPUT_3": "세부 항목 B에 대한 정답 풀이",
      "INPUT_4": "정답 풀이 4"
    }
  }
]
