...
1. 1번 문항 (첨부 이미지의 물음과 본문 HTML의 답변을 분석한 표채우기 질문) - type: "주관식 (표채우기)"
   - [우선 수행 사항]: 첨부된 문제 이미지(혹은 텍스트)를 눈으로 직접 읽고, 문제 본문에서 "구하시오", "산정하시오" 등으로 요구한 (1), (2), (3) 등의 하위 항목 텍스트를 완벽하게 타이핑하여 'ocr_text' 필드에 먼저 기록하십시오.
   ...
   - [입력칸(INPUT) 동적 분할 철칙]: 문제 원문을 분석하여 **최종적으로 구해야 하는 정답(수치)의 개수만큼 동적으로 표의 행(Row)을 생성**하십시오. 예를 들어, 어떤 지문이 "(2) A, B 및 C점에서의 간극수압"을 구하라고 했다면, 이를 1개의 행으로 뭉뚱그리지 말고 "(2) A점에서의 간극수압", "(2) B점에서의 간극수압", "(2) C점에서의 간극수압"처럼 3개의 독립된 행으로 완벽하게 분할해야 합니다. 지문이 요구하는 모든 독립된 수치마다 별도의 \`[INPUT_N]\`이 배정되도록 동적으로 분석하십시오.
...
[
  {
    ...
    "tableData": {
      ...
      "rows": [
        ["(1) 분할된 세부 항목명 1", "[INPUT_1]"],
        ["(2) 분할된 세부 항목명 2 - X점", "[INPUT_2]"],
        ["(2) 분할된 세부 항목명 2 - Y점", "[INPUT_3]"],
        ["(3) 분할된 세부 항목명 3", "[INPUT_4]"]
      ]
    },
    "answers": {
      "INPUT_1": "정답 풀이 1",
