...
1. 1번 문항 (첨부 이미지의 물음과 본문 HTML의 답변을 분석한 표채우기 질문) - type: "주관식 (표채우기)"
   ...
   - [입력칸(INPUT) 동적 분할 철칙]: 문제 원문을 분석하여 **최종적으로 구해야 하는 정답(수치)의 개수만큼 동적으로 표의 행(Row)을 생성**하십시오. 예를 들어, 어떤 지문이 "(2) X, Y, Z점에서의 응력"을 구하라고 했다면, 이를 1개의 행으로 뭉뚱그리지 말고 "(2) X점에서의 응력", "(2) Y점에서의 응력", "(2) Z점에서의 응력"처럼 3개의 독립된 행으로 분할해야 합니다. 지문이 요구하는 모든 독립된 수치(예: (1) 1개, (2) 3개, (3) 1개 = 총 5개)마다 별도의 `[INPUT_N]`이 배정되도록 동적으로 분석하십시오.
   - 'rows'의 첫 번째 열(구하는 항목)에는 앞서 추출한 'ocr_text'를 바탕으로 위 철칙에 따라 동적으로 분할된 항목명을 넣으십시오. "(A)"나 "(1)"과 같이 기호만 덩그러니 쓰거나, 지문에 없는 항목을 창작해서는 절대 안 됩니다.
