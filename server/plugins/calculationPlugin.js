...
1. 1번 문항 (첨부 이미지의 물음과 본문 HTML의 답변을 분석한 표채우기 질문) - type: "주관식 (표채우기)"
   - [우선 수행 사항 - Chain of Thought]: 최종 표를 작성하기 전에 반드시 다음 단계를 거치십시오.
     1) 'step1_extract': [필독] 첨부된 **이미지(문제 스크린샷)** 안에 적혀있는 문제 지문을 글자 하나 바꾸지 말고 그대로 눈으로 직접 읽어내십시오!
     2) 'step2_plan': 추출된 원문 중 쉼표(,)나 '및' 등으로 여러 개를 묻는 복합 항목(예: "A, B 및 C점에서의 간극수압")이 있다면, 최종적으로 수치를 계산해야 하는 독립된 단위 개수만큼 쪼개어(분할하여) 최종 항목 배열(예: "A점 간극수압", "B점 간극수압", "C점 간극수압")을 만드십시오.
   - 표 구조는 반드시 **headers: ["구하는 항목", "계산 결과 및 답안"]** 의 2열 헤더 규격으로 구성하십시오.
   - 🚨 **[절대 지침 - 창작 금지 및 원문 복사]**: 'rows'의 첫 번째 열(구하는 항목)에는 원문 텍스트(또는 분할된 텍스트)를 그대로 기재하십시오! AI 임의로 지문에 없는 "댐 저면", "양압력수두", "한계동수경사", "임의 지점의" 등의 단어를 지어내거나 요약/의역하는 것을 엄격히 금지합니다.
...
[
  {
    ...
    "ocr_text": "이미지에서 눈으로 읽어낸 문제 지문 원문 전체 텍스트 그대로 복사",
