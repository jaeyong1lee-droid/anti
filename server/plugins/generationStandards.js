export let generationStandardsList = [
  ...
  },
  {
    ...
    "content": "🚨 [수식 및 단위 표기 단권화 표정 표준 철칙 - 극도로 중요!]: 수식과 수치/단위 작성 시 `\\(` `\\)` 나 `\\[` `\\]`, `(\\ (` 같은 이중 괄호 이스케이프 및 불필요하게 꼬인 지시 표현을 철저히 금지합니다. 문장 내부에 들어가는 인라인 수식은 반드시 `$94.4\\text{mm}$` 형태로 단일 달러(`$ ... $`)를 사용하십시오. ⚠️절대로 한글 설명이나 일반 문장 전체를 달러 기호로 통째로 감싸지 마십시오. 한글 문장과 수학 기호는 철저히 분리하여, 수학 기호에만 달러 기호를 적용하십시오.",
    "updatedAt": new Date().toISOString()
  },
  {
  ...
  },
  {
    ...
    "content": "🚨 [SVG 다이어그램 크기 최적화 및 실제 수치 직접 기입 철칙 - 극도로 중요!]:\n1. [100% 꽉참 강제 금지 및 고유 크기 유지]: JSON의 `diagram_svg` 필드에 SVG 데이터를 생성할 때, `width=\"100%\"` 같은 속성을 강제하지 마십시오. 도형이나 그래프가 비정상적으로 거대해지는 것을 막기 위해 요소에 맞는 적절한 고정 픽셀(예: `width=\"400\"`) 또는 `viewBox`만 설정하여 고유의 적절한 비율을 유지하게 하십시오. 너비를 꽉 채울지 여부는 필요한 화면에서 프론트엔드가 알아서 처리합니다.\n2. [SVG 내부 수식(KaTeX) 및 텍스트 표현 규칙]: 그림(SVG) 내부에 수식 기호($s_t$, $\\sigma$ 등)나 수치를 표현해야 할 때는 항상 `<foreignObject>` 태그를 활용하여 내부에서 일반 HTML 텍스트 및 인라인 KaTeX(`$ ... $`)를 자유롭게 사용하여 깔끔하게 렌더링되도록 하십시오.\n3. [실제 수치 중심의 시각 자료 작성]: 기호(예: L_h, q)만 단독으로 표시하기보다는 지문에 주어진 **실제 수치(예: 3.0m, 45kN/m² 등)를 정확한 위치에 함께 표기**하여 수험생이 직관적으로 문제를 풀 수 있게 돕는 것이 좋습니다.\n4. [그래프 및 도해 해석 문제 출제 권장]: 문제 지문만 나열하지 말고, 모어 원(Mohr Circle), 응력 경로(Stress Path), 토압 분포도, 하중-침하 곡선 등 **시각적 그래프나 도해를 해석하여 푸는 문제**를 적극적으로 출제하십시오.\n5. [작도 순서 철칙: 지문 우선 설계]: 절대로 그림(SVG)을 먼저 대충 그려놓고 억지로 문제 지문(question)을 끼워맞추지 마십시오. **반드시 (1) 공학적 상황과 구체적인 수치가 포함된 문제 지문을 먼저 완벽하게 확정한 뒤에, (2) 그 확정된 지문의 텍스트와 수치에 100% 정확하게 들어맞는 그림을 후행적으로 작도**해야 합니다.",
    "updatedAt": new Date().toISOString()
  }
];
