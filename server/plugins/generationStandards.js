...
export let generationStandardsList = [
  ...
  },
  {
    ...
    "content": "🚨 [리드미컬하고 직관적인 공식 암기 팁(memorizationTip) 생성 의무화 - 극도로 중요!]:\n공식을 묻거나 설명하는 문항 데이터를 생성할 때, 수험생이 복잡한 전공 공식을 시험장에서 쉽게 떠올려 외울 수 있도록 돕는 재미있고 직관적인 연상식 'memorizationTip' 필드를 무조건 생성하십시오.\n[암기 팁 작성 철칙]:\n1. 너무 설명적이고 딱딱한 교과서식 서술(예: '토크 T를 파이 접시 위에 올리고 반으로 나누면...')은 절대 금지합니다.\n2. 마치 수능이나 기술사 학원의 1타 강사처럼 유치하고 병맛스럽지만 입에 착착 감기는 말장난, 콩글리시 연상, 또는 리드미컬한 두문자 리듬으로 창작하십시오.\n   - 예: Barton Q: \"알제이(RQD/Jn)가 제이아(Jr/Ja)를 만나 제이떱에스(Jw/SRF)가 되었다!\"\n   - 예: Terzaghi 극한지지력: \"씨엔씨($c N_c$) 더하기 큐엔큐($q N_q$) 더하기 반감비엔감($0.5 \\\\gamma B N_{\\\\gamma}$)\"\n   - 예: 베인 전단 강도($c_u$): \"강도 씨유($c_u$)는 티 밑에 파이디제곱($\\\\frac{T}{\\\\pi D^2}$) 괄호 열고 에이치 반 토막($H/2$) 더하기 디 여섯 토막($D/6$)! -> '씨유는 파이디제곱분의 티에, 에이치 반 디 여섯!'\"\n3. 억지로라도 입으로 읊었을 때 운율(라임)이 살아있고 입에 달라붙는 한국어 구절 형태로 문장을 창작하십시오.",
