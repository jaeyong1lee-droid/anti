export let generationStandardsList = [
  {
    ...
    "title": "1. 다열 표채우기(Table Quiz) 데이터 셀 100% 빈칸 강제 철칙",
  ...
  },
  {
    ...
    "title": "5. 객관식 계산값 정확성, 발문 종결 어미 및 정답 필드 필수 철칙",
    "content": "🚨 [객관식 계산값 정확성, 발문 종결 어미 및 정답 필드 필수 철칙 - 극도로 중요!]:\n1. [정확한 계산값 보기 주입]: 계산형 문제 출제 시, 정답과 공식 대입으로 산출되는 실제 정확한 공학적 계산값(소수점 포함)은 반드시 4개 보기 항목 중 하나로 포함시키십시오. 임의의 정수로 둥글게 만들어 '가장 근사한 값을 고르라'는 식으로 얼버무리는 행위를 엄금합니다.\n2. [변별력 오답 보기 구성]: 단순 가감산 산수 문제를 지양하고 다단계 공학 공식 대입 및 명확한 오개념에 기초한 유인 오답 보기를 설계하십시오.\n3. [발문 종결 어미]: 객관식 질문 본문(question)의 마지막 종결 어미로 절대로 '서술하십시오'를 사용하지 말고, 반드시 '알맞은 것을 고르시오', '선택하십시오'로 작성하십시오.\n4. [answer 및 correctIndex 필드 필수 포함]: 객관식 반환 JSON 객체에는 options 배열뿐만 아니라 반드시 정답 문자열 'answer'와 0 기반 정답 인덱스 'correctIndex'를 100% 필수 포함해야 합니다. answer는 options 배열의 정답 항목 문자열과 정확히 일치해야 합니다.\n5. [스키마 외 임의 필드 생성 엄금]: mathContext, extraData 등 정의되지 않은 임의의 찌꺼기 필드를 생성하지 마십시오.",
    "updatedAt": new Date().toISOString()
  },
  {
    "id": "user_generation_graph_chart_rule",
    "title": "6. 그래프 작도 기본 규칙 (차트그래프 vs SVG)",
  ...
  },
  {
    ...
    "title": "7. 아스키 아트(ASCII Art) 사용 기준",
  ...
  },
  {
    ...
    "title": "8. 특수 공법 약자(Acronym) 사전 해석 및 환각(Hallucination) 방지 철칙",
  ...
  },
  {
    ...
    "title": "9. 정답(주관식, 객관식) 하단 직관적 의미 필수 표기 철칙",
