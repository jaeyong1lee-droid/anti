export let generationStandardsList = [
  ...
  },
  {
    ...
    "title": "8. SVG 다이어그램 반응형 너비 및 하단 텍스트 배치 철칙",
    "content": "🚨 [SVG 다이어그램 창 너비 맞춤 및 텍스트 하단 배치 철칙 - 극도로 중요!]:\n1. [반응형 창 너비 맞춤]: JSON의 `diagram_svg` 필드에 SVG 데이터를 생성할 때, 고정된 픽셀 너비를 피하고 반드시 `viewBox` 속성(예: `viewBox=\"0 0 800 600\"`)과 함께 `width=\"100%\"`, `height=\"auto\"`를 설정하여 사용자 기기의 창 너비에 꽉 차게(반응형으로) 렌더링되도록 하십시오.\n2. [글자는 그림 밑에 배치]: SVG 내부에 `<text>` 요소를 포함할 경우, 복잡한 도형이나 선(그림)들을 위쪽에 우선 배치하고, 관련된 설명 텍스트나 범례 글자는 그림의 아래쪽(하단) 구역으로 일관되게 빼서 시각적으로 그림이 글자 위에 오도록 정돈하십시오.",
    "updatedAt": new Date().toISOString()
  }
];
