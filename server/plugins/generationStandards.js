export let generationStandardsList = [
  ...
  },
  {
    ...
    "content": "🚨 [SVG 다이어그램 크기 최적화 및 실제 수치 직접 기입 철칙 - 극도로 중요!]:\n1. [100% 꽉참 강제 금지 및 고유 크기 유지]: JSON의 `diagram_svg` 필드에 SVG 데이터를 생성할 때, `width=\"100%\"` 같은 속성을 강제하지 마십시오. 도형이나 그래프가 비정상적으로 거대해지는 것을 막기 위해 요소에 맞는 적절한 고정 픽셀(예: `width=\"400\"`) 또는 `viewBox`만 설정하여 고유의 적절한 비율을 유지하게 하십시오. 너비를 꽉 채울지 여부는 필요한 화면에서 프론트엔드가 알아서 처리합니다.\n2. [기호 대신 실제 수치 직접 기입 및 LaTeX 금지]: **객관식 문제를 포함한 모든 문제에서** SVG 다이어그램을 작도할 때는, 그림 내부에 기호(예: L_h, q)를 단독으로 쓰지 말고 지문에 주어진 **실제 수치(예: 3.0m, 45kN/m² 등)를 정확한 위치에 직접 텍스트로 기입**하여 수험생이 그림만 보고도 직관적으로 풀 수 있게 하십시오. 단, SVG `<text>` 태그 내부에는 절대로 `$` 또는 `\\` 기호가 들어가는 LaTeX 수식을 쓰지 마십시오(수식이 깨집니다). 수치와 기호가 필요하면 `q = 45 kN/m²` 와 같이 순수 유니코드 문자와 일반 텍스트로만 기입하십시오.\n3. [그래프 및 도해 해석 문제 출제 권장]: 문제 지문만 나열하지 말고, 모어 원(Mohr Circle), 응력 경로(Stress Path), 토압 분포도, 하중-침하 곡선 등 **시각적 그래프나 도해를 해석하여 푸는 문제**를 적극적으로 출제하십시오.\n4. [작도 순서 철칙: 지문 우선 설계]: 절대로 그림(SVG)을 먼저 대충 그려놓고 억지로 문제 지문(question)을 끼워맞추지 마십시오. **반드시 (1) 공학적 상황과 구체적인 수치가 포함된 문제 지문을 먼저 완벽하게 확정한 뒤에, (2) 그 확정된 지문의 텍스트와 수치에 100% 정확하게 들어맞는 그림을 후행적으로 작도**해야 합니다.\n5. [🚨 수식 기호($) 남용 엄격 금지]: `$` 기호는 수학 수식이나 공학 변수(예: $K_a$, $\\sigma$)에만 사용하십시오. 인명, 서명, 출처, 일반 영문 텍스트(예: Mechanics, Active, Earth 등)를 무의미하게 `$ $` 로 감싸지 마십시오.",
    "updatedAt": new Date().toISOString()
  }
];
