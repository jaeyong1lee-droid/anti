export let generationStandardsList = [
  ...
  },
  {
    ...
    "title": "8. SVG 다이어그램 반응형 너비 및 내부 텍스트 배치 철칙",
    "content": "🚨 [SVG 다이어그램 창 너비 맞춤 및 텍스트 내부 배치 철칙 - 극도로 중요!]:\n1. [반응형 창 너비 맞춤]: JSON의 `diagram_svg` 필드에 SVG 데이터를 생성할 때, 고정된 픽셀 너비를 피하고 반드시 `viewBox` 속성(예: `viewBox=\"0 0 800 600\"`)과 함께 `width=\"100%\"`, `height=\"auto\"`를 설정하여 사용자 기기의 창 너비에 꽉 차게(반응형으로) 렌더링되도록 하십시오.\n2. [글자는 그림 내부에 배치 및 LaTeX 문법 사용 금지]: SVG 내부에 `<text>` 요소를 포함할 경우, 글자를 그림 밖 하단으로 밀어내지 말고 도형이나 선(그림) 내부의 적절한 위치(레이블, 축 설명 등)에 직관적으로 배치하십시오. 단, SVG `<text>` 태그 내부에는 절대로 `$` 또는 `\\` 기호가 들어가는 LaTeX 수학 수식을 작성하지 마십시오 (웹 브라우저 렌더링 시 수식이 변환되지 않고 깨진 텍스트로 짤려서 노출됩니다). 수식 표현이 필수적이라면 `σ = P/A + M/Z` 와 같이 순수 유니코드 문자와 일반 텍스트로만 기입하십시오.",
    "updatedAt": new Date().toISOString()
  }
];
