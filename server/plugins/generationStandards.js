...
export let generationStandardsList = [
  ...
  {
    "id": "user_generation_i75zw0h",
    "title": "문제에 맞는 답변을 제공할 것",
    "content": "잘못된 예시)\n질문: 현장 시험 결과의 신뢰성을 확보하기 위해 통제해야 할 조건 / 점성토(비배수전단강도)\n- 피드백: 신뢰성 확보를 위해서는 시험 장비의 정밀도, 응력 이력 반영, 과압밀비(OCR) 고려 등 구체적인 관리 방안이 포함되어야 합니다.\n- 정답 : 현장 베인시험 또는 실내 삼축압축시험을 통한 비배수전단강도 정밀 측정\n         -> 이렇게 나오면 안되고 위 피드백에서 말하는 신뢰성 확보를 위해서는 시험 장비의 정밀도, 응력 이력 반영, 과압밀비(OCR) 고려 등 구체적인 관리 방안을 포함하여야 함."
  },
  {
    "id": "def_gen_4",
    "title": "지반공학 용어 준수 철칙",
    "content": "🚨 [지반공학 용어 준수 철칙 - 극도로 중요!]: 모든 문제, 보기, 해설 작성 시 지반공학 표준 번역 용어를 엄격히 준수하십시오. 특히 **'Flow Net'은 반드시 '유선망'**으로 번역하여 사용해야 하며, '유동망', '흐름망' 등 비표준 번역 용어를 사용하는 것을 엄격히 금지합니다. 또한 'Seepage Velocity'는 '침투속도'로 표준화하여 통일되게 작성하십시오."
  },
  {
    "id": "def_gen_5",
    "title": "단순 증감형 출제 금지 및 공학적 복합 연계 출제",
    "content": "🚨 [단순 증감형 출제 절대 금지 및 공학적 복합 연계 출제 - 극도로 중요!]: 단순히 'A가 증가하면 B가 어떻게 되는가' 또는 단순 비례/반비례 관계를 묻는 너무 쉬운 1차원적 문항의 생성을 전면 금지합니다. 대신 다음과 같이 출제하십시오:\n1. 공식의 핵심 파트(계수, 부호, 분모/분자)를 빈칸(A) 처리하는 '수식 조각 블라인드 채우기' 유형 출제 시, 반드시 질문 지문(question) 내에 전체 수식 구조를 빈칸 기호(A) 또는 (빈칸)을 포함한 디스플레이 수식($$ ... $$) 형태로 그려서 명시해야 합니다. 단순히 말로만 묻지 말고, 문제 텍스트에 전체 공식을 그려준 뒤 보기에서 고르게 하십시오. (예: 질문에 '평면 변형 조건의 등가 탄성계수 $$ E' = \\frac{E}{\\text{(A)}} $$ 공식에서 빈칸 (A)에 들어갈 조각은?' 형식으로 출제)\n2. 실제 지반 특성치 범위(예: 포화 점토의 포아송비 0.5 등)의 임계 수치를 학생이 스스로 연상한 후, 공식과 결합하여 2단계 유도 계산을 수행하도록 설계된 '한계 수치 및 물리적 직관 연계' 유형\n3. 공식이 성립하기 위한 기본 물리적 가정 조건이나 역학적 한계 상황을 직접 묻는 문항."
  }
];
