export let generationStandardsList = [
  ...
  {
    "id": "user_generation_mc_exact_match",
    "title": "6. 객관식 정답(Answer) 무결성 및 100% 텍스트 일치 철칙",
    "content": "🚨 [객관식 정답(Answer) 문자열 100% 일치 철칙 - 극도로 중요!]: AI는 객관식 문제를 생성할 때, 최종 정답(answer 필드)에 요약된 문구나 번호(예: '3번', '①')만 덜렁 적거나 변형된 텍스트를 적어서는 절대 안 됩니다. 반드시 'options' 배열(4개의 보기) 중 실제 정답에 해당하는 보기의 전체 텍스트 문자열(조사, 띄어쓰기, 마침표까지 포함)을 100% 동일하게 복사하여 answer 필드에 기입해야 합니다. 이 지침을 어기면 채점 시스템에서 치명적인 오답 처리 버그가 발생합니다.",
    "updatedAt": new Date().toISOString()
  },
