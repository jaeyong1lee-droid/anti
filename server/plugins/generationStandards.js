export let generationStandardsList = [
  ...
  },
  {
    ...
    "title": "6. 수식 기호 이스케이프 및 불필요한 꼬임 금지",
  ...
  },
  {
    ...
    "content": "🚨 [차트그래프와 SVG 다이어그램의 구분 철칙 - 극도로 중요!]:\n1. [기본 그래프는 차트그래프]: 사용자가 '그래프를 그려달라', '차트그래프를 그려달라'고 요청하면 100% **차트그래프 플러그인(Chart Plugin)**을 사용하여 `chart` 마크다운 블록 형태의 JSON 데이터를 출력하십시오. 절대 SVG로 직접 그리지 마십시오.\n2. [예외적 SVG 작도]: 단, 사용자가 명시적으로 'SVG 그래프로 그려달라', 'SVG로 그려'라고 요청한 경우에만 예외적으로 **SVG 다이어그램(SVG Plugin)**을 사용하여 작도하십시오.",
    "updatedAt": new Date().toISOString()
  },
  {
