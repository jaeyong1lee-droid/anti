export let generationStandardsList = [
  ...
  },
  {
    ...
    "content": "🚨 [표 채우기 대조축 독립성 및 절대 맵핑 철칙 - 극도로 중요!]:\n1. [가로/세로축 독립 차원 설계 의무화]: 가로 헤더(Column)와 세로 헤더(Row)는 반드시 서로 완전히 다른 고유한 공학적 평가 차원(예: 세로축 = '비교 공법/이론', 가로축 = '평가 속성/메커니즘')으로 설계하십시오.\n2. [의미적 중복 및 말장난 셀 금지]: 행(Row)과 열(Column)이 교차하는 격자 간 정답의 공학적 의미가 모호하거나 유사한 개념으로 중복 분할되는 부실한 셀 설계를 철저히 금지합니다.\n3. [좌표 기반 절대 맵핑 부여(Coordinate Mapping)]: AI 맵핑 오류를 원천 차단하기 위해, 빈칸을 뚫을 때 기존의 [INPUT_1], [INPUT_2] 방식을 버리고 반드시 **해당 격자의 물리적 좌표를 명시한 [INPUT_R{행번호}_C{열번호}] 형식**으로 아이디(ID)를 부여하십시오. (예: 1번째 행, 2번째 열 빈칸은 [INPUT_R1_C2], 2번째 행 1번째 열은 [INPUT_R2_C1]). 그리고 answers 객체의 키(Key) 역시 동일한 좌표 기반 아이디(INPUT_R1_C2 등)와 100% 매핑되도록 물리적으로 일치시키십시오.\n4. [AI 동적 출제 및 고정 템플릿 금지]: 하드코딩된 정적 템플릿 문구를 절대 강제 덮어쓰지 말고 동적으로 출제하십시오.",
    "updatedAt": new Date().toISOString()
  },
  {
  ...
  },
  {
    ...
    "title": "11. 마크다운(Markdown) 국제 표준 계층 문법 엄수 철칙",
    "content": "🚨 [마크다운 국제 표준 계층 문법 엄수 철칙 - 극도로 중요!]: 답변이나 해설(explanation) 필드에 `- ` 기호를 사용하여 불릿(Bullet) 리스트 항목을 출력할 때, 해당 항목이 상위 항목에 속하는 하위 항목(Sub-item)이라면 절대 상위 항목과 동일한 레벨로 평면화하여 나열하지 마십시오. **반드시 마크다운 국제 표준 문법에 따라 스페이스바 2칸 이상을 들여쓰기(Indentation)하여 부모-자식 간의 계층 구조를 명확히 구분하여 데이터의 무결성을 유지**하십시오. (예: `- 상위 항목` 바로 아랫줄에 `  - 하위 항목`)",
    "updatedAt": new Date().toISOString()
  }
];
