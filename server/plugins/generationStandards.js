export let generationStandardsList = [
  ...
  },
  {
    ...
    "title": "4. 주관식 서술형 고난도 및 명사형 종결어미 철칙",
    "content": "🚨 [주관식 서술형 고난도 및 명사형 종결어미 철칙 - 극도로 중요!]:\n1. [고난도 서술형 출제]: 주관식 문항(단답형 등)은 단순 용어나 명칭만을 정답으로 묻는 수준 낮은 출제를 금지하고, ① 공학적 한계점 비판, ② 공법의 실무적 단점, ③ 현장 하자 문제 상황 대처 방안을 평가하는 서술형 질문으로 출제하십시오.\n2. [명사형 종결어미 의무화]: 주관식(개요, 공식, 단답형 등) 모범 답안 어미는 평서문(~다, ~입니다)을 엄금하며, 반드시 거동 원리를 포함한 명사형 종결어미로 간결히(50~120자 내외) 작성하십시오.",
    "updatedAt": new Date().toISOString()
  },
  {
  ...
  },
  {
    ...
    "content": "🚨 [수식 및 단위 표기 단권화 표정 표준 철칙 - 극도로 중요!]: 수식과 수치/단위 작성 시 `\\(` `\\)` 나 `(\\ (` 같은 이중 괄호 이스케이프 및 불필요하게 꼬인 지시 표현을 철저히 금지하며, 반드시 `$94.4\\text{mm}$` 또는 `$94.4\\text{ mm}$` 형태로 단순 명확하게 표준 인라인 수식 문법(`$ ... $`)만 사용하여 일관성 있게 출력하십시오.",
    "updatedAt": new Date().toISOString()
  },
  {
    "id": "user_generation_strict_extraction",
    "title": "7. 수치 계산 문제 원문 문항 100% 직역 출제 철칙",
    "content": "🚨 [수치 계산 문제 원문 문항 100% 직역 출제 철칙 - 극도로 중요!]: 제공된 소스 텍스트나 이미지(OCR) 내에 '구하시오', '계산하시오' 등으로 **명시적으로 요구된 실제 하위 질문(Sub-questions)들(예: (1) 침투수량, (2) 간극수압 등)이 기재되어 있다면, AI가 묻고 싶은 전형적인 질문(예: 유선 통로 수 비율 등)을 새롭게 창작(날조)하지 마십시오.** 반드시 원문에 기재된 묻는 항목 그 자체를 토씨 하나 바꾸지 않고 100% 그대로 발췌하여 표채우기(tableData) 문항의 행(Row)으로 구성하십시오. 원문의 출제 의도를 무시하고 AI 임의로 질문을 생성하는 것을 엄금합니다.",
    "updatedAt": new Date().toISOString()
  }
];
