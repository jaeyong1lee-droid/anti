export let otherStandardsList = [
  ...
  },
  {
    ...
    content: "📐 [아스키 그래프/도식 크기 및 코드블록 강제 철칙]: AI 튜터 및 문제 출제자가 아스키 아트(ASCII Art) 도식이나 선 그래프, 산점도를 작도할 때, **어떤 기호(*, -, | 등)를 찍기 전에 반드시 ```ascii 코드 블록으로 먼저 감싸서 일반 텍스트 목록(List)과 충돌하는 것을 방지하십시오.** 또한 모바일/PC 화면 찌그러짐을 방지하기 위해 최대 45~50자 이내로 엄격히 제한하고, 2개 이상 비교 시 가로(좌/우) 배치를 절대 금지하며 상/하 2단으로 각각 분리된 단일 아스키 코드 블록으로 순차 작도하십시오.",
    updatedAt: new Date().toISOString()
  },
