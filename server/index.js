...
app.post('/api/formula/generate-memorization-tip', async (req, res) => {
  ...
  try {
    const prompt = `
당신은 대한민국 기술사 및 공학 시험을 휩쓴 1타 암기 튜터입니다.
수험생들이 시험장에서 극도의 긴장 상태에서도 이 복잡한 공학 수식을 한눈에 바로 떠올려 완벽히 쓸 수 있도록, 유치하고 병맛스럽지만 입에 착착 달라붙는 **재미있고 직관적인 공식 암기 팁(연상법) (1~2줄)**을 한국어로 창작해 주십시오.
...
[요구사항]:
1. 딱딱하고 긴 설명적 서술은 절대 금지합니다.
2. 기호(변수)의 영어 한글 발음, 운율(라임), 콩글리시 말장난을 조합하여 억지로라도 소리 내어 읊었을 때 입에 착 달라붙는 연상구절을 창작하십시오.
   - 예: Barton Q-system: "알제이(RQD/Jn)가 제이아(Jr/Ja)를 만나 제이떱에스(Jw/SRF)가 되었다!"
   - 예: Terzaghi 극한지지력: "씨엔씨($c N_c$) 더하기 큐엔큐($q N_q$) 더하기 반감비엔감($0.5 \\\\gamma B N_{\\\\gamma}$)"
   - 예: 베인 전단 강도($c_u$): "강도 씨유($c_u$)는 파이디제곱분의 티에, 에이치 반 디 여섯!"
3. 마크다운의 '\`\`\`' 등 기호는 쓰지 말고 아래 양식의 암기 팁 문장(한두 줄)만을 반환해 주십시오:
   "💡 **암기 팁**: ... 연상법: \"[입에 붙는 1타 강사식 병맛 말장난 구절]\""
4. ${LATEX_PROMPT_INSTRUCTIONS}
`;
