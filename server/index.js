...
[출제 요구사항]:
1. **1번 및 2번 문항 (정량 계산 문제)**:
   - **문제 형태**: 수험생이 직접 수식을 활용하여 수치를 연산하고 최종 답을 구해야 하는 **주관식 계산문제**여야 합니다.
   - **문제 종류 및 "type" 값**:
   ...
   - **LaTeX 수식 적용**: 질문(question), 정답(answer/answers), 해설(explanation)에 포함되는 수식 기호나 숫자는 반드시 LaTeX 기호($)로 감싸서 가독성을 확보하십시오.

2. **3번 문항 (공학적 의미/교훈 주관식 문제)**:
   - **"type" 값**: 반드시 "주관식 (단답형)"
...
3. **4번 문항 (관련 공학적 문제 발생 시 대책 주관식 문제)**:
   - **"type" 값**: 반드시 "주관식 (단답형)"
   - **질문 구성**: 이 계산 문제의 주제와 밀접하게 연관된 실무 지반공학적 문제 상황(예: 극한지지력 부족으로 인한 침하/붕괴 징후 발생, 비탈면 활동 붕괴 등)을 가정하고, 현장 기술사 관점에서의 구체적인 공학적 해결책 및 방지 대책을 묻는 질문으로 출제하십시오.
...
[🚨 절대 준수 사항]:
- **정확히 4개의 문제**만 배열에 담아 JSON 형식으로 반환하십시오. (1, 2번은 계산 문제, 3번은 의미/교훈 문제, 4번은 대책 문제)
...
[
  {
    "type": "주관식 (단답형)",
    "question": "1번 계산문제 질문 내용 (구체적인 설계 조건 수치 포함, 필요한 경우 '첨부된 [그래프/그림]을 참고하여...' 문구 삽입 또는 마크다운 표 삽입)",
    "answer": "최종 계산 결과값 (예: $125.4\\\\ \\\\mathrm{kN/m^2}$)",
    "explanation": "공식 유도 및 각 변수 대입을 통한 구체적인 계산 전개 과정 및 해설"
  },
  {
    "type": "주관식 (표채우기)",
    "question": "2번 여러 개를 입력하는 표채우기 형식의 계산문제 질문 내용",
    "tableData": {
