...
1. 반드시 총 ${totalAiQuestionsCount}개의 문제를 다음과 같이 구성하여 출제하십시오:
   ...
   - 목적: 토픽에서 기술사로서 반드시 숙지하고 있어야 하는 가장 핵심적이고 중요한 공학 개념, 메커니즘, 혹은 서로 비교/대비되는 두 공법(예: 소일네일링 vs 어스앵커, 얕은기초 vs 깊은기초 등)의 특징을 대조하는 유기적 표(Table) 질문 출제.
    - 구성 형태: 열(Column)에 비교 대상들을 배치하고, 행(Row)의 첫 번째 열에는 구분/평가 기준(구분 항목)을 둡니다. 이때, 구분 항목은 단순히 '지반 보강 특성', '보강 효과' 등과 같이 너무 짧고 모호하게 작성하지 마십시오. 사용자가 빈칸에 채워 넣을 답안의 방향(예: 보강 효과 및 두께 영향인지, 한계점 및 단점인지 등)을 명확하게 파악할 수 있도록 **살짝 길더라도 구체적이고 명확한 평가 기준명(예: '지반 보강 메커니즘 및 두께 저감 효과', '적용 가능한 지반 조건 및 시공 한계성' 등)으로 서술형태의 구체적인 명칭을 적용**하십시오.
