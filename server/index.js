...
[출제 규칙]:
1. 제공된 비교 표의 행과 열 구조를 파악하고, 구분 항목(첫 번째 열)을 제외한 모든 데이터 비교 셀(두 번째 열부터 마지막 열까지의 모든 셀)은 수험생이 직접 학습하여 채워 넣을 수 있도록 빠짐없이 전부 빈칸 \`[INPUT_행번호_열번호]\` (예: \`[INPUT_1_2]\`) 형태로 치환하여 구성하십시오.
2. 특정 비교 대상 열의 텍스트 설명을 그대로 표에 채워두어 정답 힌트를 남기는 등의 행위를 엄격히 금지합니다. 첫 번째 열(구분 항목)을 제외한 표 내부의 모든 비교 데이터 셀은 무조건 전부 \`[INPUT_]\` 토큰으로 뚫어 놓아야 합니다.
...
5. 🚨 [등방성 vs 이방성 지반 정답 엄격 분리 철칙]: 등방성 지반(kh = kv)에 관한 빈칸 정답으로 이방성 지반(kh != kv)용 수식이나 '좌표 변환' 설명을 채워 넣는 등의 심각한 공학적 오매핑을 절대 금지합니다. 등방성은 '별도 보정 없이 유선망 직접 작도/단일 투수계수 k 적용'이며, 이방성만 '좌표 변환법 적용/등가 투수계수 ke = sqrt(kh*kv) 적용'입니다. 각 열(Column) 헤더의 지질/공학적 상태(등방성 vs 이방성)에 완벽히 부합하는 설명만 answers 객체에 매핑하십시오.
6. 🚨 [가로 헤더 열 제목 설계 철칙]: 가로 헤더(tableData.headers)의 비교 대상 열 제목은 절대로 '널말뚝 하류측 분출 파괴 검토'와 같은 서술형 문장이나 모호한 단어를 적지 말고, 비교할 수 있는 구체적인 실제 대상 명칭, 시공/공법 항목(예: '널말뚝(Sheet Pile)', '댐체(Embankment Dam)')으로 고유하게 구성하십시오. 비교 대상명이 헤더에 명확히 지정되어야 두 대상의 공학적 차이를 분석하는 올바른 표채우기가 완성됩니다.
...
`;
  ...
  '  * 🚨 **[빈칸(입력 토큰) 구성 규칙 - 모든 비교 셀 빈칸 강제화 철칙]**:\n' +
  '    - 표의 구분 항목(1열 행 라벨)을 제외한 모든 비교 데이터 셀은 빠짐없이 전부 `[INPUT_1]`, `[INPUT_2]` 등의 빈칸 입력 토큰으로 비워 두어야 합니다. 일반 텍스트 설명문구나 정답을 표 내에 그대로 노출하여 힌트를 주는 행위를 엄격히 금지합니다.\n' +
...
4. 겸손하고 전문적인 대화 태도 유지:
  ...
  5. 기술사 수준의 고품격 서술형 구조 및 직관적 의미 보완 (검색 내용 기반 설명 + 직관적 설명/메커니즘/주요 가정 추가 보완 규칙):
    - [자율적 서술 구조 + 필수 보완 구성]: 답변을 작성할 때, 제공된 문서나 검색한 토질역학/지반공학 정보를 기반으로 학술적 내용을 자연스럽고 풍부하게 설명하십시오. 모든 본문을 획일적인 형식으로 통제하여 가독성을 해치지 마십시오.
    - [정의 하단 필수 보완 항목]: 단, 설명 과정에서 등장하는 각 핵심 개념, 개별 이론, 공식, 또는 공학적 수단에 대해서는 그 정의(학술적 설명) 하단에 반드시 수험생의 이해를 돕기 위해 '직관적 설명', '주요 가정', '메커니즘'을 추가로 덧붙여 작성해야 합니다. 이때 다음의 디폴트 형식(글머리기호 '• ', 볼드 '**', 콜론 ':')을 사용하여 정의 바로 아래에 삽입하십시오:
      * \`• **정의**: [해당 개념/이론의 전문적이고 학술적인 공학 용어를 사용한 정확한 정의 설명]\` (항목명 뒤에 콜론을 붙이고 한 칸 띄운 뒤 즉시 내용을 같은 줄에 작성)
      * \`• **직관적 설명**: [수험생이 시험장에서 쉽게 떠올리고 암기할 수 있는 직관적인 일상적 비유, 실무적 느낌, 혹은 외우기 쉬운 암기 팁(두문자어, 연상 기법 등)을 정의 바로 하단에 추가로 보완]\`
    - 🚨 [줄바꿈 및 소제목 첫 줄 시작 규칙]: 모든 소제목(정의, 직관적 설명, 주요 가정, 메커니즘 등)은 콜론(:) 바로 뒤에서 줄바꿈을 하지 마십시오. 반드시 콜론 뒤에 한 칸의 공백을 두고 즉시 같은 줄에 설명 문장이나 도입글을 시작하십시오. (예: '• **직관적 설명**: [내용...]' 또는 '• **주요 가정**: 다음 사항들을 전제로 함'). 그 도입글 다음 줄부터는 자유롭게 엔터를 쳐서 세부 단계나 다수의 가정 항목들을 들여쓰기된 글머리 기호(예: '  - ')로 가독성 있게 나열할 수 있습니다. 불필요한 공백 라인(빈 줄)만 남발하지 마십시오. 소제목 자체를 통째로 생략한 채 날것의 리스트로 나열하는 행위는 엄격히 금지합니다.
...
JSON 포맷 규격:
{
  ...
  "memorizationTip": "이 공식이 담고 있는 물리적/공학적 의미를 분모(Denominator)의 의미, 분자(Numerator)의 의미, 그리고 각 항(Term)의 의미로 나누어 직관적이고 친절하게 설명하십시오. 유치한 말장난이나 운율, 연상법은 절대로 쓰지 마십시오. 예시: \"💡 **직관적 의미**: 분자는 '물을 끌어올리는 표면장력 연직력'이며 분모는 '아래로 누르는 물기둥 자중'을 의미합니다. 즉 두 힘의 평형 높이를 결정합니다.\""
}
...
app.post('/api/formula/generate-memorization-tip', async (req, res) => {
  ...
  try {
    const prompt = `
당신은 대한민국 기술사 및 공학 시험 수험생을 지도하는 최고 권위의 공학 전문 튜터입니다.
수험생들이 시험장에서 복잡한 공식의 구조를 물리적으로 이해하여 기억해낼 수 있도록, **[공식의 분모의 의미], [분자의 의미], 그리고 [각 항/변수의 의미]를 물리적으로 분석한 직관적 의미(2~3줄)**를 한국어로 작성해 주십시오.
...
[요구사항]:
1. 한글 말장난이나 유치한 운율 연상법은 절대 배제하고, 공식이 담고 있는 역학적 평형이나 공학적/물리적 의미를 논리적으로 설명하십시오.
2. 예시:
   - 샌드매트 최소두께($H$): "분자($q - q_a$)는 지반이 지탱하지 못하는 '장비의 초과 접지압'을, 분모($2\\gamma\\tan\\theta$)는 '모래의 전단 마찰력을 통해 수평 방향으로 저항하는 분산력'을 뜻합니다. 즉, 초과 하중에 저항하여 버텨낼 수 있는 모래 매트 소요 두께입니다."
   - 모관상승고 ($h_c$): "분자($4T_s \\cos \\alpha$)는 '물을 위로 끌어올리는 표면장력의 연직력'을 의미하며, 분모($\\gamma_w d$)는 '모세관 내 물기둥이 아래로 누르는 자중'을 의미합니다. 즉, 두 힘의 역학적 평형에 따른 물의 상승 높이를 정의합니다."
3. 절대로 답변 내부에 큰 수식 블록($$ ... $$)을 중복하여 생성하지 마십시오. (단, $s_u$, $q_t$ 등의 인라인 수식 기호는 $ 기호를 포함하여 그대로 보존해 주십시오.)
4. 마크다운의 '\`\`\`' 등 기호는 쓰지 말고 한두 줄의 깔끔한 문장만을 반환해 주십시오:
   "💡 **직관적 의미**: [분모/분자 및 각 항의 물리적·공학적 의미에 대한 상세 분석 설명]"
5. ${LATEX_PROMPT_INSTRUCTIONS}
`;

    const responseText = await callLLMWithFailover(
      "당신은 공학 공식의 물리적 원리와 구조를 직관적으로 분석하고 설명해주는 전공 튜터입니다.",
      prompt,
      null,
      'tutor',
      { temperature: 0.7 }
