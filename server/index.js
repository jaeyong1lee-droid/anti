...
2. **2번 문항 (정량 계산 문제 - 변형 및 고난도 응용)**:
   - 1번 원보고서 기출문제의 설계 조건, 하중 상태, 수치, 매개변수(예: 가해진 하중 조건, 지반 물성치, 시험 조건 등)를 다르게 변경하여 수험생이 원리를 이해하고 응용해서 풀 수 있는 고난도 계산 문제를 출제하십시오.
   - 마찬가지로 구하고자 하는 결과가 여러 개라면 반드시 **"주관식 (표채우기)"**를 적용하여 빈칸 입력식으로 구성하고, 단 하나의 최종값만 요구하는 경우에만 "주관식 (단답형)"으로 출제하십시오.
   - 문제 풀이에 필요한 실험 수치 테이블이 필요하다면 마찬가지로 마크다운 표 형식으로 question 지문 안에 그려주십시오.
...
[
  ...
  {
    "type": "주관식 (단답형)",
    "question": "2번 변형/응용 계산문제 질문 내용 (구체적인 설계 조건 수치 포함, 필요한 경우 '첨부된 [그래프/그림]을 참고하여...' 문구 삽입 또는 마크다운 표 삽입)",
