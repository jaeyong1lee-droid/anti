...
1. 반드시 총 ${totalAiQuestionsCount}개의 문제를 다음과 같이 구성하여 출제하십시오:
   ...
   - 목적: 토픽에서 기술사로서 반드시 숙지하고 있어야 하는 가장 핵심적이고 중요한 공학 개념, 메커니즘, 혹은 서로 비교/대비되는 두 공법(예: 소일네일링 vs 어스앵커, 얕은기초 vs 깊은기초 등)의 특징을 대조하는 유기적 표(Table) 질문 출제.
    - 구성 형태: 열(Column)에 비교 대상들을 배치하고, 행(Row)의 첫 번째 열에는 구분/평가 기준(구분 항목)을 둡니다. 이때, 구분 항목은 단순히 '지반 보강 특성', '보강 효과', '실무 적용', '시험 특징' 등과 같이 너무 짧고 모호하게 작성하지 마십시오. 사용자가 빈칸에 채워 넣을 답안의 방향(예: 보강 효과 및 두께 영향인지, 한계점 및 단점인지, 실무 활용처인지, 시공 시 유의사항인지 등)을 명확하게 파악할 수 있도록 **살짝 길더라도 구체적이고 명확한 평가 기준명으로 서술형태의 구체적인 명칭을 적용**하십시오.
     - 🚨 **[구분 항목(행 제목) 명확화 원칙 - 극도로 중요!]**: 구분 항목(행 제목)은 **그것만 읽어도 어떤 종류의 답을 써야 하는지 100% 확신할 수 있어야** 합니다. 예를 들어 '실무 설계 적용 시 유의점'이라고 적으면 '실무에서 주의해야 할 점(주의사항/유의점)'인지 '실무에서 어디에 쓰이는지(활용처/적용 사례)'인지 학생 입장에서 구분이 불가합니다. 아래 예시를 참고하십시오:
       ❌ 나쁜 예(모호함): '실무 적용 특성', '보강 효과', '시험 특징', '설계 시 고려사항', '실무 설계 적용 시 유의점'
       ✅ 좋은 예(명확함): '실무에서 주로 활용되는 지반 조건 및 적용 사례', '보강재의 인장력 전달 메커니즘 및 두께 저감 효과', '시료 교란에 따른 강도 측정값의 변화 경향', '설계 시 안전율 적용 기준 및 주의할 시공 조건'
       🔍 **자기 검증 필수**: 구분 항목 작성 후 반드시 자문하십시오—**"이 구분항목만 보고 답을 쓸 때, 전혀 다른 종류의 답(예: 활용처 vs 유의점)을 적어도 맞는 것처럼 보이는가?"** → 그렇다면 구분항목이 모호한 것이므로 더 구체적으로 수정하십시오.
...
app.post('/api/question/regenerate', async (req, res) => {
  ...
  '  * 🚨 **[구분 항목(행 제목) 명확화 원칙]**: 구분 항목(rows 첫째 열)은 그것만 읽어도 어떤 답을 써야 하는지 100% 확신할 수 있어야 합니다. ❌ 나쁜 예: "보강 효과", "실무 적용" → ✅ 좋은 예: "보강재의 인장력 전달 메커니즘 및 변위 억제 효과", "실무에서 주로 활용되는 지반 조건 및 적용 사례"\n' +
