...
// Temporary endpoint to correct schedule Round 3 and Round 4 data
app.get('/api/temp-db-fix-0628', async (req, res) => {
  try {
    // 1. Topic 25: 지하수위가 존재하는 옹벽의 토압 및 수압 산정
    // Round 3 (ID 172) -> copy score, completed_at, etc. from Round 4 (ID 173)
    const sched173 = await dbQuery.get('SELECT * FROM schedules WHERE id = 173');
    if (sched173) {
      await dbQuery.run(
        `UPDATE schedules 
         SET status = 'completed', completed_at = ?, score = ?, correct_count = ?, total_count = ? 
         WHERE id = 172`,
        [sched173.completed_at, sched173.score, sched173.correct_count, sched173.total_count]
      );
      // Reset Round 4 (ID 173) to pending with planned_date 14 days from 2026-06-28 (which is 2026-07-12)
      await dbQuery.run(
        `UPDATE schedules 
         SET status = 'pending', completed_at = NULL, score = NULL, correct_count = NULL, total_count = NULL, planned_date = '2026-07-12' 
         WHERE id = 173`
      );
      // Update app_session key
      const sessionVal173 = await dbQuery.get("SELECT value FROM app_session WHERE key = 'completed_review_schedule_173'");
      if (sessionVal173) {
        await dbQuery.run("DELETE FROM app_session WHERE key = 'completed_review_schedule_172'");
        await dbQuery.run(
          "INSERT INTO app_session (key, value, updated_at) VALUES ('completed_review_schedule_172', ?, CURRENT_TIMESTAMP)",
          [sessionVal173.value]
        );
        await dbQuery.run("DELETE FROM app_session WHERE key = 'completed_review_schedule_173'");
      }
    }

    // 2. Topic 27: 유선문제
    // Round 3 (ID 184) -> copy score from Round 4 (ID 185)
    const sched185 = await dbQuery.get('SELECT * FROM schedules WHERE id = 185');
    if (sched185) {
      await dbQuery.run(
        `UPDATE schedules 
         SET status = 'completed', completed_at = ?, score = ?, correct_count = ?, total_count = ? 
         WHERE id = 184`,
        [sched185.completed_at, sched185.score, sched185.correct_count, sched185.total_count]
      );
      // Reset Round 4 (ID 185) to pending with planned_date 14 days from 2026-06-28 (which is 2026-07-12)
      await dbQuery.run(
        `UPDATE schedules 
         SET status = 'pending', completed_at = NULL, score = NULL, correct_count = NULL, total_count = NULL, planned_date = '2026-07-12' 
         WHERE id = 185`
      ...
      // Update app_session key
      const sessionVal185 = await dbQuery.get("SELECT value FROM app_session WHERE key = 'completed_review_schedule_185'");
      if (sessionVal185) {
        await dbQuery.run("DELETE FROM app_session WHERE key = 'completed_review_schedule_184'");
        await dbQuery.run(
          "INSERT INTO app_session (key, value, updated_at) VALUES ('completed_review_schedule_184', ?, CURRENT_TIMESTAMP)",
          [sessionVal185.value]
        );
        await dbQuery.run("DELETE FROM app_session WHERE key = 'completed_review_schedule_185'");
      }
    }

    res.json({ success: true, message: 'Database corrected successfully.' });
  } catch (err) {
    console.error(err);
...
`;
          ...
          // -------------------------------------------------------------
          // Batch 2 Prompt: Table Fills (표채우기 3문항) -> 총 3문항
          // -------------------------------------------------------------
          const promptBatch2 = `
[🚨 최우선 절대 준수 법규 (Constitutional Guidelines) - 작업을 시작하기 전에 가장 먼저 확인하고 100% 준수하십시오]:
당신은 대한민국 국가기술자격 기술사(Professional Engineer) 시험 출제위원으로서 문제를 출제하기 전, 아래 명시된 **문제생성 절대 지침들**과 **공학적 이론 기준**을 헌법의 제1조 철칙으로 삼아 이를 먼저 완벽하게 숙지하고 절대적으로 복종하여 문제를 설계 및 출제해야 합니다. 지침을 위반하여 출제된 문제는 시스템 검증 단계에서 즉시 폐기됩니다.
