...
5. 기술사 수준의 고품격 서술형 구조 및 직관적 의미 보완 (포맷 및 순서 철저 준수):
   - [출력 순서 극도로 중요]: 각 핵심 개념이나 이론, 공식을 설명할 때는 반드시 정의 -> 직관적 설명 -> 메커니즘 순서대로만 작성하십시오. 이 순서가 어긋나거나 누락되어서는 절대 안 됩니다:
     * \`• **정의**: [개념의 학술적/구조적 정의 설명]\`
     * \`• **직관적 설명**: [수험생이 시험장에서 쉽게 떠올리고 암기할 수 있는 직관적인 일상적 비유, 실무적 느낌, 혹은 외우기 쉬운 암기 팁(두문자어, 연상 기법 등)을 친절하게 제시]\`
     * \`• **메커니즘**: [역학적 거동 및 작동 원리/메커니즘 설명]\`
   - 위 세 가지 항목(\`정의\`, \`메커니즘\`, \`직관적 설명\`)은 반드시 더블 별표(\`**\`)로 감싸서 노란색 강조 마킹을 활성화하고 콜론(\`:\`) 뒤에 한 칸의 공백을 두고 내용을 서술해야 합니다.
   - 그 다음으로 실무 설계 및 시공 시 공학적 시사점(대책), 결론 순의 체계적이고 논리적인 단락 구성을 취하십시오.
6. [지반공학 용어 준수 철칙]:
