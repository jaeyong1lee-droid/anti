...
1. 반드시 총 ${totalAiQuestionsCount}개의 문제를 다음과 같이 구성하여 출제하십시오:
   ...
   - ⚠️ **[비교 컬럼 빈칸 처리 원칙 - 극단적으로 중요]**: 만약 비교표가 3개 이상의 열로 구성되어 2개 이상의 공법/상황을 비교하는 경우(예: 정적 하중 운용 vs 동적 및 편심 하중 운용), 일부 비교 컬럼을 빈 텍스트로 두어 완전히 누락시키는 것을 금지합니다. **모든 비교 대상 컬럼의 셀에 빠짐없이 순차적으로 `[INPUT_1]`, `[INPUT_2]`, `[INPUT_3]`, `[INPUT_4]` 등의 입력 토큰을 채워 넣으십시오.** 빈칸 (A), (B), (C), (D) 등과 같이 질문 지문에도 이 개수를 맞추어 명시하십시오. 만약 비교 대상 열이 비어 있어 아무것도 입력할 수 없거나 비교 대상이 단 하나뿐이라면 2개 열(구분, 비교대상1)만으로 표를 구성하고, 3개 이상의 열로 구성할 때는 모든 비교 셀을 빈칸 토큰으로 채워 입력창으로 만들어야 합니다.
   - "type" 값: 반드시 "주관식 (표채우기)"
   - "question": 표의 빈칸에 알맞은 핵심 답안을 서술하라는 질문 (예: "다음 소일네일링과 어스앵커 공법의 주요 공학적 특징 비교표 빈칸 (A), (B), (C), (D)에 들어갈 내용을 기술하십시오."). (⚠️ [지문 작성 수칙 - 매우 중요!]): "question" 본문에는 절대로 "INPUT_1", "INPUT_2" 또는 "[INPUT_1]" 같은 시스템 토큰명 자체를 노출하여 적지 마십시오. 대신 사용자가 직관적으로 알아볼 수 있도록 순서대로 "(A)", "(B)", "(C)", "(D)" 등으로 지칭하여 지문을 구성하십시오.
