...
JSON 포맷 규격:
{
  ...
  "memorizationTip": "이 공식을 수험생이 쉽게 연상하여 외울 수 있도록 돕는 유치하고 병맛스럽지만 입에 착 붙는 1타 강사식 한글 말장난/라임(운율) 위주의 연상 암기법(1~2줄)입니다. 절대 딱딱하고 긴 설명식 문장은 피하십시오. 예시: \"💡 **암기 팁**: ... 연상법: \\\"알제이(RQD/Jn)가 제이아(Jr/Ja)를 만나...\\\"\""
}
...
app.post('/api/formula/generate-memorization-tip', async (req, res) => {
  ...
  try {
    const prompt = `
당신은 대한민국 기술사 및 공학 시험 수험생을 지도하는 최고 권위의 1타 암기 튜터입니다.
수험생들이 시험장에서 복잡한 공식의 형태를 쉽게 기억하고 정확히 유도해낼 수 있도록, **[직관적인 공학적/물리적 비유]와 [입에 붙는 리드미컬한 한글 말장난/연상법]이 아름답게 결합된 실용적인 암기 팁(1~2줄)**을 한국어로 창작해 주십시오.
...
[요구사항]:
1. 억지스러운 개그에만 치우치는 것은 금지하며, 공식의 비례/반비례 관계나 물리적 배경을 직관적으로 이해시키는 팁과 리드미컬한 한국어 소리 연상법을 조화롭게 작성하십시오.
2. 예시:
   - 샌드매트 최소두께($H$): "분자는 '장비 접지압에서 지반 지지력을 뺀 남는 하중($q - q_a$)', 분모는 '모래가 수평으로 퍼져나가는 힘($2\\gamma\\tan\\theta$)'으로 기억하면 직관적입니다!"
   - Barton Q-system: "RQD는 Jn으로 나누고 Jr은 Ja로 나누며 Jw는 SRF로 나눕니다. 연상식: **'알제이(RQD/Jn)가 제이아(Jr/Ja)를 만나 제이떱에스(Jw/SRF)가 되었다!'**"
   - Terzaghi 극한지지력: "점착력($c N_c$) + 상재압($q N_q$) + 자중($0.5 \\gamma B N_{\\gamma}$) 세 성분의 합! 입에 붙게 **'씨엔씨 더하기 큐엔큐 더하기 반감비엔감'**으로 암기합시다."
3. 절대로 암기 팁 내부에 큰 수식 블록($$ ... $$)을 한 번 더 생성하지 마십시오. (단, $s_u$, $q_t$ 등의 인라인 수식 기호는 $ 기호를 포함하여 그대로 보존해 주십시오.)
4. 마크다운의 '\`\`\`' 등 기호는 쓰지 말고 한두 줄의 직관적인 암기 팁 문장만을 반환해 주십시오:
   "💡 **암기 팁**: [공학적 비유 또는 외우기 쉬운 아이디어 및 리드미컬 연상법]"
