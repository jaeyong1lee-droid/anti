...
app.get('/api/dashboard', async (req, res) => {
  ...
  try {
    ...
    const sql = `
      ...
      ORDER BY s.planned_date ASC, CASE WHEN s.review_round = 99 THEN 0 ELSE 1 END ASC, s.review_round ASC, t.id ASC
    `;
...
1. 반드시 총 ${totalAiQuestionsCount}개의 문제를 다음과 같이 구성하여 출제하십시오:
   ...
   - 개수: 반드시 정확히 6개의 객관식 문제를 출제해야 합니다.
    - [단순 증감/대입형 출제 절대 금지 - 매우 중요]:
      * 단순히 'A가 증가하면 B가 어떻게 되는가' 또는 단순 비례/반비례 관계를 묻는 1차원적인 단순 증감 질문은 절대로 출제하지 마십시오.
      * 또한, 질문 지문에 공식이나 수치를 미리 다 제시한 뒤 '이 값을 대입해 단순히 푸시오' 식의 단순 계산 문제도 절대로 출제하지 마십시오.
      * 대신, 6개의 객관식 중 최소 3개 이상을 아래의 고난도 유형으로 반드시 구성하십시오:
        1. [수식 조각 블라인드(빈칸) 채우기]: 공식의 일부분(핵심 계수, 부호, 또는 분모/분자 파트)을 빈칸 (A) 또는 (빈칸) 처리하되, 반드시 질문 지문(question) 내에 전체 수식 구조를 빈칸 기호를 포함하여 디스플레이 수식($$ ... $$) 형태로 친절하게 그려서 명시하십시오. 단순히 텍스트 설명으로만 묻지 말고, 문제 텍스트에 전체 공식을 그려준 뒤 보기에서 고르게 해야 합니다. (예: G = E / (A) 형태를 지문에 명시)
        2. [한계 수치 및 물리적 직관 연계형]: 지반의 특정 임계/극한 상태(예: 완전 비압축성 포화 점토의 포아송비가 0.5라는 조건 등)를 학생이 스스로 파악한 후, 이를 관계식에 대입하여 최종 결과값이나 비 비율(예: G/E의 비)을 도출하게 하는 고차원 2단계 계산/추론 문제.
        3. [기본 가정 및 역학적 한계 질문]: 공식이 유도되는 물리적/수학적 가정 조건(예: 선형탄성, 등방성 등)이나 설계 기준(KDS) 상의 한계 적용 범위를 묻는 지문형 문제.
