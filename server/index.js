...
- "type" 값: 반드시 "객관식 (4지선다)"
- "question": 구체적이고 학술적인 내용 일치, 원리 분석 또는 공식 분석/정량적 계산 객관식 질문. (서술형 문제, 공식/수식 이해형 문제, 정량적 수치 계산 문제를 균등하게 고려하여 생성하십시오)
- "options": 4개의 보기 문항으로 구성된 문자열 배열 (반드시 정답 1개와 매력적인 오답 3개로 구성).
- "answer": "options" 배열 안에 있는 값 중 정확히 일치하는 정답 문자열.
- "explanation": 왜 이 보기가 정답이고 다른 보기들이 오답인지에 대한 논리적이고 전문적인 상세 해설.
...
2. 객관식 문제의 유형을 고루고루 분배하여 출제하십시오:
   - **서술형/이해형 문제**: 약 40%
   - **공식/수식 자체의 형태 및 물리적 의미 분석 문제**: 약 30% (예: 계수 산정식, 토압 공식, 침하량 공식 등의 LaTeX 형태 및 매개변수 관계 파악)
   - **정량적/간이 수치 계산 문제**: 약 30% (예: 제공된 소스 텍스트 내의 공식을 기반으로 간단한 수치 대입 계산을 푸는 계산형 문제)
   - 반드시 공식 및 정량적인 문제가 균등하고 고루고루 분배되도록 출제해야 합니다.
...
2. 객관식 문제의 유형을 고루고루 분배하여 출제하십시오:
   - **서술형/이해형 문제**: 약 40%
   - **공식/수식 자체의 형태 및 물리적 의미 분석 문제**: 약 30% (예: 계수 산정식, 토압 공식, 침하량 공식 등의 LaTeX 형태 및 매개변수 관계 파악)
   - **정량적/간이 수치 계산 문제**: 약 30% (예: 제공된 공식에 간단한 수치를 대입하여 푸는 계산형 문제)
   - 반드시 공식 및 정량적인 문제가 골고루 분배되도록 출제해야 합니다.
3. 오답 보기 구성 주의사항 (매우 중요):
   - 오답 보기(options) 구성 시 **절대로 터무니없거나 극단적인 표현, 혹은 비현실적인 공학적 가정(예: '무한대로 상승시킴', '실시간으로 기하급수적으로 증가함', '영원히 변하지 않음', '아예 발생하지 않음', '폭발함' 등)은 절대로 사용하지 마십시오**. 
   - 실제 전공 서적이나 실무 기술 기준에 부합하는 **고도로 타당성 있고 그럴듯한 오답(plausible engineering distractors)**으로 구성해 주십시오. 모든 보기는 반드시 원본 소스 및 공학적 상식선에 긴밀히 결합되어야 합니다.
