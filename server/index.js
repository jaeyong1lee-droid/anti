...
import { ENGINEERING_STANDARDS, standardsList, updateLiveEngineeringStandards } from './plugins/calculationPlugin.js';
import { GENERATION_STANDARDS, generationStandardsList, updateLiveGenerationStandards } from './plugins/generationPlugin.js';
...
3. **3번 문항 (공학적 의미/교훈 주관식 문제)**:
   ...
   - **정답("answer")**: 채점관이 납득할 수 있는 핵심적인 공학적 설명 문장(1~2줄, 30자 내외)으로 작성하고, 중요한 필수 공학 키워드는 마크다운 강조 기호인 **키워드**로 감싸서 작성하십시오.
...
4. **4번 문항 (관련 공학적 문제 발생 시 대책 주관식 문제)**:
   ...
   - **정답("answer")**: 구체적인 현장 대책(지반 개량 공법, 하중 저감 대책, 보강 공법 적용, 차수/배수 공법 등)을 명확하게 1줄 서술형(30자 내외)으로 작성하고, 중요한 필수 공학 키워드는 마크다운 강조 기호인 **키워드**로 감싸서 작성하십시오.
...
app.get('/api/engineering-standards', async (req, res) => {
  try {
    ...
    const standardsFilePath = path.join(__dirname, 'plugins', 'calculationPlugin.js');
    const content = await fs.promises.readFile(standardsFilePath, 'utf-8');
    const match = content.match(/export let standardsList = (\[[\s\S]*?\]);/);
    if (!match) {
      return res.status(500).json({ error: 'standardsList structure not found in calculationPlugin.js' });
    }
...
app.post('/api/engineering-standards', async (req, res) => {
  try {
    ...
    // 3. Try to save to file system (works locally, silently bypassed on Vercel read-only system)
    try {
      const filePath = path.join(__dirname, 'plugins', 'calculationPlugin.js');
      const fileContent = await fs.promises.readFile(filePath, 'utf-8');
      const updatedContent = fileContent.replace(
        /export let standardsList = \[\s*[\s\S]*?\n\];/m,
        `export let standardsList = ${JSON.stringify(standards, null, 2)};`
      );
      await fs.promises.writeFile(filePath, updatedContent, 'utf-8');
      console.log('Successfully wrote engineering standards to local file (calculationPlugin.js).');
    } catch (fsErr) {
      if (fsErr.code === 'EROFS') {
...
app.post('/api/grading-standards', async (req, res) => {
  try {
    ...
    // 3. Try to save to file system (local environment only)
    try {
...
app.post('/api/generation-standards', async (req, res) => {
  try {
    ...
    try {
      const filePath = path.join(__dirname, 'plugins', 'generationPlugin.js');
      const fileContent = await fs.promises.readFile(filePath, 'utf-8');
      const updatedContent = fileContent.replace(
        /export let generationStandardsList = \[\s*[\s\S]*?\n\];/m,
        `export let generationStandardsList = ${JSON.stringify(standards, null, 2)};`
      );
      await fs.promises.writeFile(filePath, updatedContent, 'utf-8');
      console.log('Successfully wrote generation standards to local file (generationPlugin.js).');
    } catch (fsErr) {
      if (fsErr.code === 'EROFS') {
        console.log('Read-only file system detected (Vercel). Bypassed local file write.');
      } else {
