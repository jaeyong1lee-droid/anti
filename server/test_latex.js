...
// 오브젝트 딥 힐러 트리구조
function healDeep(obj, parentKey = null, context = null) {
...
function healTheoryQuestionObject(t) { return healDeep(t); }
function healFormulaQuestionObject(f) { return healDeep(f); }
function healAnswersheetQuestionObject(a) { return healQuizQuestionObject(a); }
...
const LATEX_CHAT_PROMPT_INSTRUCTIONS = `
[🚨 극도로 중요한 LaTeX 수식 및 마크다운 렌더링 절대 준수 수칙]:
...
`;
function isCalculationQuestion(q, topicCategory = null) {
  if (!q) return false;
...
console.log(transformLatexContent('포아송비($\\nu$)'));
console.log(transformLatexContent('*  = \\frac{E}{2(1 + \\nu)}$'));