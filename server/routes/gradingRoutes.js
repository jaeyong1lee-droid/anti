...
1. **[페어형 빈칸 출제 의무화]**: 이번 재생성 문제에서는 흐름도 전체 상자 개수의 **정확히 40% (소수점 반올림)** 에 해당하는 상자 개수(예: 총 7개 상자인 경우 **3개 상자**)를 비워야 합니다. 비울 때는 반드시 선택한 상자의 **"단계 제목"과 "세부 내용" 세트를 통째로 비우십시오.** 
   - 이로 인해 빈칸 기호는 **(A), (B), (C), (D), (E), (F)의 총 6개**가 뚫리게 됩니다.
   - 첫 번째 비워진 박스: [ (A) ] 와 - (B)
   - 두 번째 비워진 박스: [ (C) ] 와 - (D)
   - 세 번째 비워진 박스: [ (E) ] 와 - (F)
...
4. **[기존 빈칸 및 정답의 100% 원천 배제]**: 
   ...
   - 이번 재생성에서는 **기존에 뚫려 있던 위 단계 정답 단어들을 절대로 정답 타겟으로 삼지 마십시오.** 즉, 이전 문제에서 비워두었던 상자는 이번 문제에서는 설명 텍스트를 고스란히 복원해 두고, **이전에 비어있지 않고 차있던 상자 중 3개를 새로 골라 빈칸(A)~(F)으로 비워야 합니다.**
5. **[동적 tableData/answers 스키마]**: 뚫어낸 빈칸의 개수(총 6개)에 맞추어 tableData의 rows와 answers의 INPUT도 정확히 6개(INPUT_1부터 INPUT_6까지)로 동적 구성하여 JSON을 출력하십시오.
`;
      }
