...
1. **[빈칸 수량 산정 공식]**: 이번 재생성 문제에서는 흐름도 전체 상자 개수의 **정확히 40% (소수점 반올림)** 에 해당하는 상자 개수를 빈칸으로 뚫어야 합니다. (예: 상자가 총 7개인 경우, 7 * 0.4 = 2.8 -> **3개**의 상자를 비워서 각각 [ (A) ], [ (B) ], [ (C) ] 로 빈칸을 배치하십시오.)
2. **[기존 빈칸 및 정답의 100% 원천 배제]**: 
   - 기존의 빈칸 정답들은 다음과 같습니다: [ ${prevAnswers || '없음'} ]
   - 이번 재생성에서는 **기존에 뚫려 있던 위 단계 정답 단어들을 절대로 정답 타겟으로 삼지 마십시오.** 즉, 이전 문제에서 비워두었던 상자는 이번 문제에서는 설명 텍스트를 고스란히 채워두고, **이전에 채워져 있던 상자 중 3개를 새로 골라 빈칸으로 뚫어야 합니다.**
3. **[동적 tableData/answers 스키마]**: 뚫어낸 빈칸의 개수(M개)에 맞게 tableData의 rows와 answers의 INPUT도 정확히 M개(INPUT_1부터 INPUT_M까지)로 동적 구성하여 JSON을 출력하십시오.
`;
      }
