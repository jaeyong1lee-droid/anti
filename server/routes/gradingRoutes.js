...
3. **[🚨 70자 가로폭 닫힌 박스 규격 원상복구]**: 모든 상자의 크기는 가로 폭을 **정확히 동일한 70칸 너비**로 통일하여 닫힌 박스(Closed Box) 형태를 취하게 하십시오. 한글은 2자, 영어/기호/공백은 1자로 정밀 계산하여 세로선을 칼정렬해야 합니다.
5. **[🚨 기존 빈칸 및 정답의 100% 원천 배제 및 강제 빈칸 셔플링]**: 
   - 기존 문제의 빈칸 정답 단어들은 다음과 같습니다: [ ${prevAnswers || '없음'} ]
   - 이번 재생성에서는 **기존에 뚫려 있던 위 단계(단어들)를 절대로 다시 뚫어서는 안 됩니다!!!**
   - 이전 문제에서 빈칸(A), (B), (C), (D) 등으로 비어 있던 상자들은 이번 문제에서는 텍스트 설명 및 단계명을 100% 온전하게 글자 하나 안 빼놓고 글씨를 다 채워서 복원해 두고,
   - **이전 문제에서 빈칸이 아니었던 다른 상자(즉, 이전 문제에서 텍스트가 가득 채워져 노출되어 있던 상자)들 중에서 M개를 골라 완전히 새롭게 빈칸으로 뚫어야 합니다.**
   - 만약 이전과 동일한 상자 단계를 빈칸으로 다시 출제하는 게으른 복사 행위를 할 경우 시스템 필터링에 의해 문제 생성이 취소되므로, 반드시 빈칸의 위치(상자 단계)를 완전히 새롭게 교체하십시오.
6. **[🚨 절대 빈칸 기호 중복 금지 철칙]**: (A), (B), (C), (D) 등의 기호를 뚫을 때, 실수로 **(C)를 두 번 쓰거나 하는 기호 중복 할당을 절대 금지합니다.** 반드시 (A), (B), (C), (D) 순서대로 각각 1번씩만 고유하게 사용하여야 합니다.
7. **[동적 tableData/answers 스키마]**: 뚫어낸 빈칸의 개수(총 6개)에 맞추어 tableData의 rows와 answers의 INPUT도 정확히 6개([INPUT_R1_C2] 등 좌표 기반)로 동적 구성하여 JSON을 출력하십시오.
8. **[🚨 상자 내부 기술사급 상세 내용 기입 철칙 - 간소화 금지]**: 
...
[🚨 재생성 필수 요구사항]:
기존 소스 문제에서 비워두었던 정답 단어([ ${prevAnswers || '없음'} ])들이 위치한 상자 단계는 텍스트를 그대로 다 복원하여 채워두십시오. 그리고 기존 소스 문제에서 빈칸이 아니었던 다른 상자들을 새로 골라 (A), (B), (C) 등의 빈칸으로 뚫어 완전히 다른 위치의 흐름도 빈칸 문제를 출제하십시오.`;
...
- **12번형태 (개념 및 거동원리 중심)**:
  ...
  * 🚨 [서술형 질문 구성 필수 규칙 - 극도로 중요!]: 단순히 단어/명칭을 단답식으로 묻는 질문(예: "~ 조건은?", "~ 공법 명칭은?", "~ 조건이라 하는가?")은 절대로 출제하지 마십시오. 반드시 "~하는 역학적 기전을 설명하시오", "~하는 거동 특성을 서술하십시오"와 같이 문장 형태의 서술형 답변을 유도하는 질문으로 구성하십시오.
  * 🚨 [정답(answer) 서술형 작성 필수 규칙 - 극도로 중요!]: 정답(answer)은 단답형 단어나 명칭이 될 수 없으며, 반드시 50자~120자 내외의 구체적이고 전문적인 지반공학 메커니즘이 포함된 완성된 서술형 문장이어야 합니다.`;
        } else {
          subtypeInstruction = `
- **13번형태 (현장 하자/위해 시나리오 및 실무 대책 중심)**:
  ...
  * 🚨 [서술형 질문 구성 필수 규칙 - 극도로 중요!]: 단순히 단어/명칭을 단답식으로 묻는 질문은 절대로 금지하며, 반드시 "~한 대책을 기술하십시오", "~한 대처 방안을 수립하여 설명하시오"와 같이 구체적인 서술을 요구하는 질문으로 구성하십시오.
...
아래 문제의 질문 지문과 보기를 정밀 분석하여, 올바른 정답을 도출하고 해설을 작성하십시오.
...
[검증 및 연산 수칙]:
1. 문제를 분석하여 제시된 보기 중 가장 적절한 정답을 찾으십시오.
${isCalc ? `2. 지문의 모든 단위를 정확히 환산하여 대입하고, "mathExpression" 필드에 최종 계산할 순수 산술 수식(예: "(4 * 0.0728) / (9810 * 0.00008)")을 작성하십시오.` : ''}
3. "correctAnswer" 필드는 제시된 보기 목록(Options) 중 정답에 해당하는 보기 문자열 그대로 지정하십시오.
4. 해설(explanation) 작성 시, 절대 "0번 보기", "1번 보기" 와 같이 개발자용 0-based 인덱스 번호를 해설 텍스트에 출력하지 마십시오. 반드시 "첫 번째 보기", "①번 보기" 또는 보기의 내용을 직접 인용하여 설명하십시오.
5. 해설(explanation)에는 [적용 이론/공식], [보기 분석 및 검증], [최종 정답 산출]을 포함하여 명확하고 논리적으로 서술하십시오.
