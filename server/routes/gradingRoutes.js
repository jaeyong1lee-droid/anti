...
4. 이중 아래첨자(예: \\Delta_h_w, \\sigma_v_0) 절대 금지: $\\Delta h_w$ (공백 분리) 또는 $\\Delta_{hw}$처럼 중괄호({})로 묶거나 공백으로 분리하십시오.
`;
...
[🚨 흐름도 문제 재생성 특수 철칙 (기존 빈칸 복원 및 새 빈칸 셔플링)]:
1. 기존 문제의 빈칸 정답 단어들은 다음과 같습니다: [ ${prevAnswers || '없음'} ]
2. 이번 재생성에서는 기존에 뚫려 있던 위 단계(단어들)를 절대로 다시 뚫어서는 안 됩니다.
3. 이전 문제에서 빈칸(A), (B), (C), (D) 등으로 비어 있던 상자들은 이번 문제에서는 텍스트 설명 및 단계명을 100% 온전하게 글자 하나 안 빼놓고 다 채워서 복원해 두고, 이전 문제에서 빈칸이 아니었던 다른 상자들 중에서 새롭게 빈칸을 골라 뚫으십시오.
`;
...
- **13번형태 (현장 하자/위해 시나리오 및 실무 대책 중심)**:
  ...
  * 🚨 [정답(answer) 서술형 작성 필수 규칙 - 극도로 중요!]: 정답(answer)은 단답형 단어가 아닌, 현장 대책과 공학적 원리가 포함된 50자~120자 내외의 완성된 서술형 문장이어야 합니다.`;
        ...
        formatRequirement = `{"type": "객관식 (4지선다)", "question": "질문", "tableData": null, "options": ["보기1", "보기2", "보기3", "보기4"], "answer": "보기1", "correctIndex": 0, "explanation": "해설 (🚨 반드시 마지막에 📚 참조 문헌(Standards & Literature) 출처 2~3개 명시)"}`;
      }
