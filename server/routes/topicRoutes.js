...
router.get('/topics', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: '서버 오류로 토픽 목록을 조회하지 못했습니다.', details: error.message || String(error) });
  }
});
...
router.get('/dashboard', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: '서버 오류로 복습 대시보드를 불러올 수 없습니다.', details: error.message || String(error) });
  }
});
