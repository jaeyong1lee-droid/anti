...
async function writeStandardToFile(fileName, list) {
  ...
  try {
    const filePath = path.join(serverDir, 'plugins', fileName);
    let codeContent = `// This file is auto-generated by the system. Do not edit manually.
`;
    
    if (fileName === 'generationStandards.js') {
      codeContent += `export let generationStandardsList = ${JSON.stringify(list, null, 2)};
...
`;
    } else if (fileName === 'lockscreenStandards.js') {
      codeContent += `export let lockscreenStandardsList = ${JSON.stringify(list, null, 2)};
...
`;
    } else if (fileName === 'engineeringStandards.js') {
      codeContent += `export let standardsList = ${JSON.stringify(list, null, 2)};
...
`;
    ...
    await fs.promises.writeFile(filePath, codeContent, 'utf-8');
    console.log(`[File Sync] Wrote updated standards to ${fileName}`);
  } catch (err) {
    console.error(`[File Sync Error] Failed to write to ${fileName}:`, err.message);
  }
}
  ...
  try {
    ...
    if (JSON.stringify(merged) !== JSON.stringify(dbList)) {
      try {
        ...
        console.log('[Sync] Automatically synced grading standards to database.');
      } catch (saveErr) {
        console.error('Failed to auto-save merged grading standards to database:', saveErr.message);
      }
    }
    updateLiveGradingStandards(merged);
    res.json({ standards: merged });
  } catch (err) {
  ...
  try {
    ...
    if (JSON.stringify(merged) !== JSON.stringify(dbList)) {
      try {
        ...
        console.log('[Sync] Automatically synced generation standards to database.');
      } catch (saveErr) {
        console.error('Failed to auto-save merged generation standards to database:', saveErr.message);
      }
    }
    updateLiveGenerationStandards(merged);
    res.json({ standards: merged });
  } catch (err) {
  ...
  try {
    ...
    if (JSON.stringify(merged) !== JSON.stringify(dbList)) {
      try {
        ...
        console.log('[Sync] Automatically synced lockscreen standards to database.');
      } catch (saveErr) {
        console.error('Failed to auto-save merged lockscreen standards to database:', saveErr.message);
      }
    }
    updateLiveLockscreenStandards(merged);
    res.json({ standards: merged });
  } catch (err) {
...
JSON 반환 포맷:
{
  ...
  "structure": "이 공식에 사용된 각 기호(변수)들의 정의. 반드시 각 변수 기호는 $로 감싸고 '- $기호$: 정의' 형태의 마크다운 불릿 목록으로 줄바꿈하여 작성하십시오 (예: '- $t$: 계측 경과 시간\\n- $t_0$: 기준 시간\\n- $\\alpha$: 초기 접선 기울기 역수 상수').",
...
JSON 포맷 규격:
{
  ...
  "analysis": "해당 그림/그래프에 표현된 다양한 구성 요소, 변수 관계, 공학적 의미 및 거동 메커니즘을 상세히 설명하십시오.",
  ...
  try {
      ...
      return formulaQuestions.map(q => {
        ...
        if (isSeepageTarget) {
          ...
          return {
            ...q,
            title: "필터 및 배수/차수 설계",
            question: "댐체 및 기초지반의 침투류 제어 대책 중 차수 대책과 배수 대책의 거동 메커니즘을 아래 비교표의 빈칸에 맞게 서술하시오.",
            tableData: {
          ...
          return {
            ...q,
            title: "말뚝 하중 전이 메커니즘",
            question: "말뚝 기초 하중 전이 메커니즘 중 주면 전단 거동(t-z)과 선단 저항 거동(q-z)의 거동 특징을 아래 비교표의 빈칸에 맞게 서술하시오.",
            tableData: {
    ...
    const latestStandards = [
      ...
      },
      {
        ...
        "title": "주관식 정답의 장문 메커니즘 의무화",
        "content": "🚨 [주관식 정답의 장문 메커니즘 의무화 - 극도로 중요!]: 주관식(개요, 공식, 단답형, 표채우기 등)의 모든 모범 답안(\"answers\" 내의 각 값 또는 \"answer\")은 절대로 1~2 단어의 단순 명칭이나 짧은 요약형 문장으로 작성해서는 안 되며, 반드시 지반공학적 거동 원리, 인과관계, 시공 및 설계 제어 메커니즘을 명확히 명시하되, 너무 길어지지 않도록 핵심 위주의 명료한 서술형(최소 50자에서 최대 120자 내외)으로 간결하게 작성하십시오."
      },
      {
