...
2. 2번 문항 (개념 비교 표 칸채우기 문제) - type: "주관식 (표채우기)"
   ...
   - headers: ["구분 항목", "비교대상 1", "비교대상 2", "(비교 대상 3이 있다면 추가)"]
...
[응답 JSON 포맷]:
반드시 아래 지정된 JSON 배열 포맷으로만 정확히 반환하십시오. 2번 문항(표채우기)은 비교 대상이 3개 이상일 경우 반드시 예시처럼 모든 데이터 칸을 [INPUT_N]으로 뚫어내야 합니다.
\`\`\`json
[
  {
    "type": "주관식 (표채우기)",
  ...
  {
    "type": "주관식 (표채우기)",
    "question": "2번 문항: 다음 (비교 대상 공법명) 공법들의 주요 특징 비교표 빈칸에 들어갈 내용을 서술하시오.",
    "tableData": {
      "headers": ["구분 항목", "비교대상 1", "비교대상 2", "(비교 대상 3이 있다면 추가)"],
      "rows": [
        ["평가 항목 1", "[INPUT_1]", "[INPUT_2]", "[INPUT_3]"],
        ["평가 항목 2", "[INPUT_4]", "[INPUT_5]", "[INPUT_6]"]
      ]
    },
    "answers": {
      "INPUT_1": "비교대상 1 서술",
...
` : `
[문제 생성 태스크 시작]:
아래 제공되는 정보를 분석하여 총 정확히 13개의 예상문제를 생성해 주십시오. (객관식 3개, 그래프해석 주관식 1개, 개요 1개, 공식 1개, 표채우기 2개, 단답형 5개)
[토픽 핵심 주제]: ${coreSubject}
[토픽 원본 제목]: ${topic.title}
...
---------------------------------------------------------
[문제 생성 태스크 시작]:
위의 절대 지침과 기준 법규를 완전히 숙지한 상태에서, 아래 제공되는 [토픽 핵심 주제], [핵심 키워드], [첨부파일 본문 텍스트]를 심층 분석하여, 총 **정확히 8개**의 예상문제(주관식 개요 1개, 주관식 공식 1개, 주관식 표채우기(흐름도) 1개, 주관식 그래프해석 1개, 주관식 단답형 4개)를 생성해 주십시오.
...
[출제 요구사항]:
반드시 총 8개의 문제를 다음과 같이 구성하여 출제하십시오:
...
- "structure": 🚨 **[필수사항]** 개요 설명 및 공식에 등장하는 모든 기호(예: $K_a$, $\\phi$, $\\delta$ 등)의 정의를 반드시 작성하십시오. 기호가 전혀 없는 경우에는 빈 문자열("")로 작성하십시오.
...
- "structure": 🚨 **[필수사항]** 위 formula에서 사용된 모든 기호의 정의를 장황하지 않게 최소한의 명사형 위주로 반드시 작성하십시오.
...
- 출제 원칙: 
  ...
  * answers 객체의 각 INPUT 키("INPUT_1"부터 "INPUT_2*M"까지)에 들어갈 정답은 간결하게 작성하여 수험생이 명료하게 채점받을 수 있게 설계하십시오.

[4번 문제] 주관식 (그래프해석):
- 목적: 토픽의 공학적 거동이나 상관관계를 그래프로 분석하는 고품격 문제.
- "type" 값: 반드시 "주관식 (그래프해석)"
- "question": 토픽과 관련된 핵심 상관관계 그래프 질문 텍스트 아래에 그래프 차트 JSON 삽입 (\`\`\`chart {...}\`\`\`).
  * 🚨 [데이터 조밀도 2배 증가 지침]: 차트 데이터 배열(data)을 생성할 때 X축의 간격을 2배 이상 촘촘하게(조밀하게) 쪼개어 구성하십시오. 점(Data point)의 개수를 최소 8개 이상으로 생성하여, 곡선의 경향성이 훨씬 부드럽고 정밀하게 표현되도록 하십시오.
- "answer": 그래프가 시사하는 공학적 의미나 설계/시공 시 주의사항을 기술사 수준으로 서술.
- "explanation": 그래프 해석의 상세 논리 설명.

[주관식 (단답형) 문제들 (5, 6, 7, 8번 문제)]:
- 개수: 반드시 정확히 4문제를 출제하십시오.
- "type" 값: 반드시 "주관식 (단답형)"
- 🚨 [객관식/선택형 옵션(보기) 제공 절대 금지 규칙 - 극도로 중요!]: 주관식(개요, 공식, 단답형, 표채우기, 그래프해석)의 그 어떤 문항에서도 객관식용 보기(options, 예: ①, ②, ③, ④ 등 또는 "options" 필드)를 절대로 설계하거나 기입하여 제공하지 마십시오. 모든 주관식 문항은 오직 서술형 정답만을 요구해야 합니다.
- 출제 원칙:
  * **[🚨 간결하고 핵심을 찌르는 질문 설계]**: 문제를 출제할 때 지문(question) 안에 정답을 유추할 수 있는 너무 상세한 현장 상황이나 공학적 기전, 조건들을 장황하게 전부 나열하여 정답을 스포일러(유출)하는 행위를 엄격히 금지합니다.
  * **[출제 유형 예시]**:
    1. 핵심 공학적 원리나 한계점을 묻는 질문 (예: "~~ 이론의 기본 가정이 내포하는 실무적 한계는 무엇인가?")
    2. 실무 적용 시 주요 단점이나 대책을 묻는 질문 (예: "~~ 공법 적용 시 지반 침하를 방지하기 위한 핵심 대책은 무엇인가?")
    - 지문(question)은 2~3줄 이내로 간결명료하게 작성하여, 수험생이 스스로 현장 상황과 역학적 원리를 떠올려 답안(answer)에 상세히 서술할 수 있도록 유도하십시오.
  * **정답("answer")**: 모범 답안은 단순히 한 단어 키워드가 아니라, 구체적인 공학적 거동 메커니즘과 설계/시공 시 인과관계 대책이 논리적으로 상세히 포함된 서술형(최소 50자에서 최대 120자 내외)으로 명료하게 작성하십시오. 또한, 이 정답 문장 내에서 채점에 중요도가 가장 높은 필수 공학 키워드들은 반드시 역슬래시 없이 일반 마크다운 강조 기호인 **키워드** 형태로 감싸서 작성해 주십시오. (예: **이중층 두께**, **전단강도 저하** 등)
  * **[🚨 정답 하단 직관적 의미 필수 추가 철칙 - 극도로 중요!]**: 모든 주관식 모범 답안(answer) 맨 하단에는 반드시 줄바꿈 후 '💡 **직관적 의미**: [해당 공학적 거동의 핵심 본질이나 실무적 의미를 한눈에 직관적으로 단번에 이해할 수 있도록 쉽게 비유하거나 요약한 1~2문장]'을 필수로 포함하십시오.
...
[
  ...
  },
  {
    ...
    "answers": {
      "INPUT_1": "(1)의 올바른 정답 문구",
      "INPUT_2": "(2)의 올바른 정답 문구",
      "INPUT_3": "(3)의 올바른 정답 문구",
      "INPUT_4": "(4)의 올바른 정답 문구"
    },
...
- 목적: 해당 [토픽 핵심 주제]와 밀접하게 연관된 **대표적인 기법 비교, 공법 비교, 이론 비교** 등 서로 대비되는 핵심 대상을 선정하고, 이들의 역학적 특징, 거동 기전, 또는 장단점을 명확하게 대조하는 유기적인 비교표(Table) 채우기 질문을 출제해 주십시오. (예: 액상화 방지 대책에서 SCP공법과 모래다짐공법 비교, 혹은 정적 액상화와 동적 액상화 이론 비교 등)
  ...
  - 🚨 **[데이터 셀 100% 빈칸 강제 철칙 - 극도로 중요!]**: 행 제목(첫 번째 열)을 제외한 모든 데이터 셀을 100% 전부 빈칸([INPUT_1], [INPUT_2]...)으로 뚫어내야 합니다. 일부 칸만 비우고 나머지를 텍스트로 채우는 행위는 절대 금지합니다. 
  ...
  - 🚨 **[각 빈칸 정답 하단 직관적 의미 필수 추가 철칙 - 극도로 중요!]**: 각 INPUT의 모범 답안 끝에는 반드시 줄바꿈 후 '💡 **직관적 의미**: [해당 셀 항목의 핵심 거동이나 실무적 본질을 한눈에 알기 쉽게 요약한 1~2문장]'을 필수로 포함하십시오.
...
[응답 JSON 포맷]:
반드시 아래 지정된 JSON 배열 포맷으로만 정확히 반환하십시오. 마크다운의 \`\`\`json 코드 블록이나 추가적인 텍스트 설명은 배제하고 순수한 JSON 데이터만 제공해 주십시오. 비교 대상이 3개 이상일 경우 반드시 예시처럼 모든 데이터 칸을 [INPUT_N]으로 뚫어내야 합니다.
[
  {
    "type": "주관식 (표채우기)",
    ...
    "tableData": {
      "headers": ["구분 항목", "비교대상 1", "비교대상 2", "(비교 대상 3이 있다면 추가)"],
      "rows": [
        ["평가 항목 명칭", "[INPUT_1]", "[INPUT_2]", "[INPUT_3]"],
        ["평가 항목 명칭", "[INPUT_4]", "[INPUT_5]", "[INPUT_6]"]
      ]
    },
    "answers": {
      "INPUT_1": "비교대상 1 서술",
  ...
  },
  {
    ...
    "tableData": {
      ...
      "rows": [
        ["평가 항목 명칭", "[INPUT_1]", "[INPUT_2]"],
        ["평가 항목 명칭", "[INPUT_3]", "[INPUT_4]"]
      ]
    },
    "answers": {
...
---------------------------------------------------------
[문제 생성 태스크 시작]:
위의 절대 지침과 기준 법규를 완전히 숙지한 상태에서, 아래 제공되는 [토픽 핵심 주제], [핵심 키워드], [첨부파일 본문 텍스트], [이전 회차 오답 정보], [사용자 피드백 지침] 그리고 [사용자 문제 조정 내역]을 심층 분석하여, 총 **정확히 3개**의 예상문제(객관식 4지선다 3개)를 생성해 주십시오.
${specialInstructions}
${weaknessPrompt}
${feedbackPrompt}
${adjustmentsPrompt}
...
[출제 요구사항]:
반드시 총 3개의 객관식 문제를 다음과 같이 구성하여 출제하십시오:

- 목적: 토픽의 상세한 원리, 메커니즘, 장단점 등을 다각도로 평가하는 고난도 4지선다형 질문.
- "type" 값: 반드시 "객관식 (4지선다)"
- [계산문제 비중 조건 - 매우 중요]: 전체 3개의 객관식 문제 중, 반드시 정확히 1개의 문제는 공학적 수치 판단이나 정량적 분석 능력을 평가하는 문제로 출제하십시오. 단, 질문 지문에 공식이나 수치를 미리 제시한 뒤 "이 값을 대입하여 계산하시오" 식의 기계적 계산 문제는 절대로 출제하지 마십시오.
...
let parsedArray = null;
    ...
    const normalizedParsedArray = (parsedArray || []).map(q => {
      ...
      if (type === '주관식') {
        ...
        else if (subtype === '그래프해석') { newType = '주관식 (그래프해석)'; newSubtype = '그래프해석'; }
      } else if (type === '개요') {
        newType = '주관식 (개요)';
        newSubtype = '개요';
      } else if (type === '공식') {
...
1. 이번 회차에서는 **정확히 5개의 문제**만 반환하되 다음 유형별로 각각 정확히 1문제씩 골고루 구성하여 비율을 사수하십시오:
   ...
   - 주관식 (type: "주관식 (그래프해석)"): 1문제 (기술사 시험에 도움이 될 수 있도록 토픽의 공학적 거동이나 상관관계를 꿰뚫어 보여주는 분석적 인터랙티브 차트(Chart Plugin) JSON 블록(\`\`\`chart ... \`\`\`)을 질문(question) 텍스트의 **맨 마지막**에 반드시 포함하고, 수험생이 이 그래프를 분석하여 서술형으로 답하게 하는 고품격 그래프 해석 주관식 문제)
2. 그래프 해석 주관식 출제 지침 (극도로 중요):
   - 해당 토픽의 핵심을 가장 잘 보여줄 수 있는 유의미한 변수 간의 관계(예: 심도별 응력 분포, 하중-침하 곡선, 응력 경로 등)를 동적 인터랙티브 차트로 도출하십시오.
   - 🚨 [데이터 조밀도 2배 증가 지침]: 차트 데이터 배열(data)을 생성할 때 X축의 간격을 2배 이상 촘촘하게(조밀하게) 쪼개어 구성하십시오. 점(Data point)의 개수를 최소 8개 이상으로 생성하여, 곡선의 경향성이 훨씬 부드럽고 정밀하게 표현되도록 하십시오.
   - 차트 작성 시 Chart Plugin의 마크다운 블록 문법(\`\`\`chart JSON... \`\`\`)을 정확히 준수하여 "question" 필드 내용의 마지막에 삽입하십시오.
   - 모범답안(answer)은 단순 수치 나열이 아닌, 그래프가 시사하는 공학적 의미나 설계/시공 시 주의사항을 기술사 수준으로 서술해야 합니다.
   ...
   - 특히 **수치 해석법이나 가설 구조물 해석과 같이 정량적 분석이 필요한 토픽의 경우, 제공된 소스 문서 내에 명시적인 수치나 파라미터가 존재한다면 이를 활용하여 정량적인 그래프(차트)를 구성하십시오. 단, 문서에 수치나 수식이 없다면 임의로 비현실적인 수치를 가상 부여하지 말고 정성적인 거동 경향 차트로 대체하십시오.**
...
3. 소스 텍스트의 숨겨진 공학적 개념과 실무 기전을 포착하여 고품격 질문을 던지십시오.
...
[
  ...
  {
    "type": "주관식 (그래프해석)",
    "topic_title": "이 문제의 출제 근거가 되는 토픽 목록 내의 정확한 토픽명",
    "question": "공학적 현상 분석 질문 텍스트\\n\\n\`\`\`chart\\n{ \\\"type\\\": \\\"line\\\", ... }\\n\`\`\`",
    "tableData": null,
    "options": [],
    "answer": "그래프에 대한 기술사급 분석 및 공학적 의미 서술 모범답안",
...
다음 규칙을 엄격히 따르십시오:
1. 보기 편한 Markdown 형식(적절한 굵은 글씨, 글머리 기호 등)을 사용하되, 마크다운 코드블록(\`\`\`markdown)으로 전체를 감싸지 말고 바로 텍스트로 출력하십시오.
