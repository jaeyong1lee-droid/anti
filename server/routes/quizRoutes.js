...
function assembleFinalQuestions(questions, topic, carryOverQuestions, fileText) {
  ...
  const finalShorts4 = [];
  conceptQs.forEach(q => finalShorts4.push(q));
  fieldQs.forEach(q => finalShorts4.push(q));
...
- "structure": (선택사항) 개요 설명에 공식 기호 정의가 필요할 때만 작성하십시오.
...
- 🚨 **[구분 항목(행 제목) 명확화 원칙 - 극도로 중요!]**: 구분 항목(행 제목)은 그것만 읽어도 이 표가 무슨 주제/토픽에 대한 비교인지, 이 행에 어떤 종류의 답을 써야 하는지 직관적으로 이해할 수 있어야 합니다.
- 🚨 **[모범 답안-구분항목 범주 일치 원칙 - 극도로 중요!]**: 각 INPUT의 모범 답안은 반드시 해당 행의 구분 항목(행 제목)이 요구하는 답변 범주에 정확히 부합하는 내용이어야 합니다.
- answers 객체의 각 INPUT 키("INPUT_1"부터 "INPUT_4"까지)에 들어갈 정답은 명사형 종결어미로 간결하게 작성하여 수험생이 명료하게 채점받을 수 있게 설계하십시오.
...
let parsedArray = null;
    ...
    if (topic.category === '계산') {
      ...
      const [batch1Text, batch2Text, batch3Text] = await Promise.all([
        localCallLLM(systemInstruction, promptBatch1, null, 'question', { temperature: 1.0 }),
        localCallLLM(systemInstruction, promptBatch2, null, 'question', { temperature: 1.0 }),
        localCallLLM(systemInstruction, promptBatch3, null, 'question', { temperature: 1.0 })
      ]);
