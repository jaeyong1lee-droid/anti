  ...
  return [
    {
      type: "주관식 (표채우기)",
      subtype: "표채우기",
      question: `[${title} 계산 문제] 첨부 그림의 상이한 조건 (a), (b)에 따른 지반의 허용지지력 및 허용하중을 계산하여 아래 표의 빈칸을 완성하시오.`,
      tableData: {
      ...
      answers: {
        INPUT_1: "조건(a) 허용지지력 풀이 및 계산값",
        INPUT_2: "조건(b) 허용지지력 풀이 및 계산값",
        INPUT_3: "조건(a) 허용하중 풀이 및 계산값",
        INPUT_4: "조건(b) 허용하중 풀이 및 계산값"
      },
      explanation: "원보고서 및 제공된 스크린샷 이미지의 공학적 설계 조건(지반 종류, 지하수위, 기초폭 등)을 대입하여 극한 지지력을 계산하는 전개 과정입니다."
    },
    {
      type: "주관식 (표채우기)",
      subtype: "표채우기",
      question: `[${title} 이론 비교] ${title} 관련 대표 이론/공법 간의 핵심 파괴 메커니즘 및 고려사항 차이점을 비교하는 표의 빈칸을 서술하시오.`,
      tableData: {
        headers: ["구분 항목", "대표 이론 A (Terzaghi 등)", "대표 이론 B (Meyerhof 등)"],
        rows: [
          ["파괴 메커니즘 특성", "[INPUT_1]", "경사/偏心 하중 고려 전단 파괴 확장"],
          ["지상/지중 지보 고려 범위", "기초 저면 상부 지반 자중 중량만 고려", "[INPUT_2]"]
        ]
      },
      answers: {
      ...
      explanation: "공학적 지지력 이론 간의 가정 사항 및 실무적 적용 범위 차이점을 비교 설명하는 문항입니다."
    },
    {
      type: "주관식 (단답형)",
...
1. 1번 문항 (첨부 이미지의 물음과 본문 HTML의 답변을 분석한 표채우기 질문) - type: "주관식 (표채우기)"
   ...
   - 🚨 **[헤더 명칭 간결화 철칙]**: headers에는 절대 콜론(:)이나 구하는 항목 명칭을 덧붙이지 말고, 오직 `"구분"`, `"조건 (a)"`, `"조건 (b)"` (또는 `"Case 1"`, `"Case 2"`)와 같이 단순하고 명료한 조건명만 기재하십시오! (예: `"조건 (a): 순극한지지력"` (X) -> `"조건 (a)"` (O))
   - headers: ["구분", "조건 (a)", "조건 (b)"]
