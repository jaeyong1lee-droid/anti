...
- 출제 원칙:
  * **[🚨 플로우차트 문제 적용 조건]**: 만약 이 토픽이 프로세스/절차적 특징 또는 공학적 거동 메커니즘(Mechanism), 단계별 파괴/변형 전개 과정 등의 흐름적 성격이 존재하여 위 "토픽 맞춤형 아스키 플로우차트 주관식 (표채우기) 문제 출제 의무화 지침"의 적용 대상인 경우 (특히 '2차원 침투 흐름과 Laplace 방정식' 토픽인 경우 '유선망 작도 및 침투 해석 단계별 흐름도' 등 흐름적 성격이 있으므로 적용 대상임), **단답형 4문제 중 최소 1문항은 반드시 해당 지침에 따른 아스키 플로우차트 주관식 (표채우기) 문제(질문 안에 백틱으로 묶은 플로우차트가 있고, tableData와 answers 객체 구조를 100% 갖춘 형태)로 출제하십시오.**
