...
- "structure": 🚨 **[필수사항]** 개요 설명에 등장하는 모든 공식 기호(예: $K_a$, $\\phi$, $\\delta$ 등)의 정의를 줄바꿈(\\n)으로 구분하여 반드시 작성하십시오. 공식이나 기호가 없는 경우에는 빈 문자열("")로 작성하십시오. (예: "- $K_a$: 주동토압계수\\n- $\\phi$: 흙의 내부마찰각")
...
- "structure": 🚨 **[필수사항]** 위 formula에서 사용된 모든 기호의 정의를 장황하지 않게 줄바꿈(\n)으로 최소한의 명사형 위주로 반드시 빠짐없이 작성하십시오. 절대로 생략하거나 빈 문자열로 두어서는 안 됩니다. (예: "- $t$: 숏크리트 두께\n- $P$: 지반압\n- $C$: 점착력")
