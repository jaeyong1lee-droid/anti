...
1. 1번 문항 (첨부 이미지의 물음과 본문 HTML의 답변을 분석한 표채우기 질문) - type: "주관식 (표채우기)"
   🚨 **[1번 문항 필수 구조 지침]**:
   - 본문 HTML을 심층 분석하여 이미지 문제에서 구하는 항목들(예: 허용지지력, 허용하중, 극한지지력 등)과 조건별 케이스(예: 조건(a), 조건(b), Case 1, Case 2 등)를 파악하십시오.
   - 조건 케이스를 **열(Column)**에, 구하는 항목을 **행(Row)**에 배치하는 2×N 또는 M×2 구조의 표로 설계하십시오.
   - 표의 모든 칸(첫 번째 열 제외)은 INPUT으로 처리되어야 합니다.
   - 🚨 아래 JSON 스키마 예시를 반드시 따르십시오 (케이스 수/항목 수는 본문 분석 결과에 맞게 조정):
   {
     "type": "주관식 (표채우기)",
     "question": "아래 그림 조건 (a), (b)에 대하여 각각 허용지지력과 허용하중을 구하여 표를 완성하시오.",
     "tableData": {
     ...
     "answers": {
       "INPUT_1": "조건(a)의 허용지지력 계산값 및 풀이",
       "INPUT_2": "조건(b)의 허용지지력 계산값 및 풀이",
       "INPUT_3": "조건(a)의 허용하중 계산값 및 풀이",
       "INPUT_4": "조건(b)의 허용하중 계산값 및 풀이"
     },
     "explanation": "상세 계산 풀이 과정"
   }
...
2. 2번 문항 (개념 비교 표 칸채우기 문제) - type: "주관식 (표채우기)"
   ...
   - 🚨 아래 JSON 스키마 예시를 반드시 따르십시오:
   {
     "type": "주관식 (표채우기)",
     "question": "다음 [공법/이론 A]와 [공법/이론 B]의 특징 비교표에서 빈칸을 채우시오.",
     "tableData": {
       "headers": ["구분 항목", "[공법/이론 A명]", "[공법/이론 B명]"],
       "rows": [
         ["비교항목 1", "[INPUT_1]", "채워진 내용"],
         ["비교항목 2", "채워진 내용", "[INPUT_2]"],
         ["비교항목 3", "[INPUT_3]", "채워진 내용"]
       ]
     },
     "answers": {
       "INPUT_1": "공법/이론 A의 비교항목1 내용",
       "INPUT_2": "공법/이론 B의 비교항목2 내용",
       "INPUT_3": "공법/이론 A의 비교항목3 내용"
     },
     "explanation": "비교표 상세 해설"
   }
