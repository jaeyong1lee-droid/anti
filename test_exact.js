...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 514: ', processed);
  // [Self-Healing] Convert misplaced double dollar ($$) math inside parentheses or sentence flow to single dollar ($) inline math
  processed = processed.replace(/\(([^()\n]*?)\$\$\s*([\s\S]*?)\s*\$\$\s*([^()\n]*?)\)/g, '($1 $$$2$$ $3)');
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 516: ', processed);
  processed = processed.replace(/([([\uAC00-\uD7A3a-zA-Z0-9,])\s*\$\$\s*([^\$\n]+?)\s*\$\$\s*([)\],\.\uAC00-\uD7A3a-zA-Z0-9,])/g, '$1 $$$2$$ $3');
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 517: ', processed);
  // [Self-Healing] Fix misplaced dollar signs inside parentheses like (s_{\infty}$) -> ($s_{\infty}$)
  processed = processed.replace(/\(([^$()\n]+?)\$\)/g, '($$$1$)');
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 519: ', processed);

  // [Self-Healing] Fix corrupted HTML entity inequality symbols (&\lt;, &amp;\lt;, etc.)
  processed = processed.replace(/&amp;\\?lt;?/gi, '<')
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 522: ', processed);
                       .replace(/&amp;\\?gt;?/gi, '>')
                       .replace(/&\\lt;?/gi, '<')
                       .replace(/&\\gt;?/gi, '>')
                       .replace(/&lt;/gi, '<')
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 530: ', processed);
                       .replace(/W유m?/gi, '\\mu m');
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 537: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 542: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 545: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 550: ', processed);
  processed = processed.replace(/\\\s*Delta\s*([a-zA-Z])\b/gi, '\\Delta $1');
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 551: ', processed);

  // [Self-Healing] Strip KaTeX-unsupported MathJax \pu{...} commands (renders red in KaTeX)
  processed = processed.replace(/\\pu\s*\{([^}]+)\}/gi, '$1');
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 554: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 557: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 567: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 588: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 600: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 605: ', processed);

  // Replace Won symbol (₩) with backslash (\) to restore LaTeX commands
  processed = processed.replace(/₩/g, '\\');
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 608: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 618: ', processed);

  // Replace Greek unicode letters and standalone words with LaTeX commands
  processed = processed.replace(/β/g, '\\beta')
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 621: ', processed);
                       .replace(/α/g, '\\alpha')
                       .replace(/γ/g, '\\gamma')
                       .replace(/σ/g, '\\sigma')
                       .replace(/τ/g, '\\tau')
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 651: ', processed);
  processed = processed.replace(/(?<!\\)\b(Delta|Sigma|Gamma|Phi|Theta|Omega)\b/g, '\\$1');
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 652: ', processed);

  // Parse root patterns
  processed = replaceRoots(processed);
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 655: ', processed);

  // Restore LaTeX commands corrupted by JSON escape sequence parsing (e.g. \neq -> \x0a + eq)
  processed = processed.replace(/\x0a\s*eq\b/g, '\\neq')
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 658: ', processed);
                       .replace(/\x0a\s*e\b/g, '\\ne')
                       .replace(/\x0a\s*u\b/g, '\\nu')
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 674: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 685: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 708: ', processed);

  

  if (!isNested) {
    processed = htmlTableToMarkdown(processed, null);
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 713: ', processed);
    processed = wrapMarkdownTables(processed);
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 714: ', processed);
  }
  ...
  processed = processed.replace(/(?<=:[^\n]*)\s+([–—−-]\s*(?:\$[^\$]+\$|[a-zA-Z0-9_\\\{\}]+)\s*:)/g, '\n$1');
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 720: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 724: ', processed);
    processed = processed.split('\n').map(line => {
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 725: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 732: ', processed);
  // Collapse double or multiple backslashes before % to single backslash
  processed = processed.replace(/\\{2,}%/g, '\\%');
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 734: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 737: ', processed);
  processed = processed.replace(/(\\quad\s*\\text\{[a-zA-Z]+\}|\b[a-zA-Z]+\b|\b\\text\{[a-zA-Z]+\})\s*\$(\s*_[a-zA-Z0-9])/g, '$$ $1$2');
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 738: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 747: ', processed);
    if (string[offset - 1] === '\\') {
      return match;
    }
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 758: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 778: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 787: ', processed);
...
if (processed && processed.includes('$$내부')) console.log('$$ appeared at line 793: ', processed);
    if (token.type === 'table') {
