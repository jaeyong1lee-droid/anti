...
const targetStr = `function extractVariablesFromMath(mathContent) {
...
const index = content.indexOf('function extractVariablesFromMath');
if (index === -1) {
  console.error('Could not find start of target string in server/index.js!');
  process.exit(1);
}

// We will find the end of target block by searching for the end of post route for suggest-title
const endRouteStr = `  } catch (err) {
    console.error('Formula suggest title route error:', err);
    res.status(500).json({ error: '서버 오류가 발생했습니다.' });
  }
});`;

const endIndex = content.indexOf(endRouteStr, index);
if (endIndex === -1) {
  console.error('Could not find end of target suggest-title route in server/index.js!');
  process.exit(1);
}
...
const updatedContent = content.substring(0, index) + replacement + content.substring(endIndex + endRouteStr.length);
