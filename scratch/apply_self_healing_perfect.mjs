...
// 3. Review Subjective
// Matches from `{/* Detailed Answer Button */}` until the closing `</div>` of that block perfectly.
const reviewSubjRegex3 = /\{\/\* Detailed Answer Button \*\/\}\s*<div className="mt-3 pt-2 border-t border-slate-700\/50">[\s\S]*?✨ 답안 전문보기 \(AI 심층 해설\)[\s\S]*?<\/div>\s*<\/div>\s*\)\}\s*<\/div>/;
...
// 4. Exam MC Button
const examMcBtnRegex = /\{\/\* 답안 전문보기 버튼 \*\/\}\s*\{!detailedAnswers\[idx\]\?\.text[\s\S]*?✨ 답안 전문보기 \(AI 심층 해설\)[\s\S]*?<\/button>\s*\)\}/g;
const examMcBtnReplacement = `{/* 문제조정 버튼 */}
...
// 5. Exam MC Result
const examMcResultRegex = /\{\/\* 답안 전문보기 결과 \*\/\}[\s\S]*?detailedAnswers\[idx\]\?\.loading[\s\S]*?<\/div>\s*<\/div>\s*\)\}/g;
...
// 6. Exam Subjective
// Matches from `{/* Detailed Answer Button & Content */}` until the closing `</div>` of that block perfectly.
const examSubjRegex = /\{\/\* Detailed Answer Button & Content \*\/\}\s*<div className="mt-3 pt-2 border-t border-slate-700\/50">[\s\S]*?✨ 답안 전문보기 \(AI 심층 해설\)[\s\S]*?<\/div>\s*<\/div>\s*\)\}\s*<\/div>/;
...
if (reviewMcBtnRegex.test(content)) {
  ...
  count++;
  console.log("Replaced Review MC Button");
} else {
  console.log("Failed to find Review MC Button");
}

if (reviewMcResultRegex.test(content)) {
  content = content.replace(reviewMcResultRegex, reviewMcResultReplacement);
  count++;
  console.log("Replaced Review MC Result");
} else {
  console.log("Failed to find Review MC Result");
}

if (reviewSubjRegex3.test(content)) {
  content = content.replace(reviewSubjRegex3, reviewSubjReplacement);
  count++;
  console.log("Replaced Review Subjective block");
} else {
  console.log("Failed to find Review Subjective block");
}

if (examMcBtnRegex.test(content)) {
  content = content.replace(examMcBtnRegex, examMcBtnReplacement);
  count++;
  console.log("Replaced Exam MC Button");
} else {
  console.log("Failed to find Exam MC Button");
}

if (examMcResultRegex.test(content)) {
  content = content.replace(examMcResultRegex, examMcResultReplacement);
  count++;
  console.log("Replaced Exam MC Result");
} else {
  console.log("Failed to find Exam MC Result");
}

if (examSubjRegex.test(content)) {
  content = content.replace(examSubjRegex, examSubjReplacement);
  count++;
  console.log("Replaced Exam Subjective block");
} else {
  console.log("Failed to find Exam Subjective block");
}

fs.writeFileSync(filePath, content, 'utf8');
console.log(`Self-healing finished! Replaced ${count} blocks.`);
