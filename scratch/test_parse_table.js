...
function parseQuestionTable(q) {
  ...
  if (questionText.toLowerCase().includes('<table') || questionText.toLowerCase().replace(/\s+/g, '').includes('<table')) {
    ...
    if (match) {
      ...
      console.log("No Match for /<table>([\\s\\S]*?)<\\/table>/i in cleaned string:", cleaned);
    }
  } else {
    console.log("Includes checks failed");
  }

  return { questionText, tableData };
}

const res = parseQuestionTable({ question: questionText });
console.log("Resulting questionText:", res.questionText);
console.log("Resulting tableData:", res.tableData);
