...
function healLatexFormulas(text) {
  ...
  // Step 0: Strip outer dollars containing Korean text
  ...
  // Step 1: Wrap larger equations (with =, <, >)
  ...
  let tokens = tokenizeForHealing(text);
  tokens.forEach(tok => {
    if (tok.type === 'text') {
  ...
  let healed = tokens.map(t => t.content).join('');
  ...
  // Step 3: Re-tokenize and run subscript/greek letter wrapping ONLY on text tokens
  tokens = tokenizeForHealing(healed);
  tokens.forEach(token => {
    if (token.type === 'text') {
      let t = token.content;

      // Wrap standard math words/symbols with backslash
      const mathWords = [
...
console.log("=== FINAL HEALED OUTPUT ===");
