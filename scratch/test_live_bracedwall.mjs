...
console.log('=== Starting Live Verification for Braced Wall & Elasto-plastic Method Routing ===');

try {
  // 1. Get all topics to find the target topic ID for "흙막이" or "탄소성"
  ...
  const bracedWallTopic = topics.find(t => 
    (t.title || '').includes('흙막이') || 
    (t.keywords || '').includes('흙막이') ||
    (t.title || '').includes('탄소성')
  );
  
  if (!bracedWallTopic) {
    console.log('Warning: No live topic containing "흙막이" or "탄소성" found.');
    console.log('Available topics:');
    topics.forEach(t => console.log(`- [ID: ${t.id}] ${t.title} (keywords: ${t.keywords})`));
    
    console.log('Aborting verification as no "흙막이" topic is registered on this environment.');
  } else {
    console.log(`Found target topic: [ID: ${bracedWallTopic.id}] "${bracedWallTopic.title}"`);
    
    // 2. Trigger AI Questions generation for the topic (this will invoke the Interceptor)
    console.log(`Triggering AI Questions for topic ${bracedWallTopic.id}...`);
    const quizRes = await fetch(`https://anti-ashy.vercel.app/api/topics/${bracedWallTopic.id}/ai-questions`, {
      method: 'POST',
    ...
    if (quizStatus === 200) {
      ...
      console.log('\n--------------------------------------------------');
      if (hasBoiling) {
        console.error('FAIL: Detected residual boiling/compaction topics in the generated braced wall set!');
      } else {
