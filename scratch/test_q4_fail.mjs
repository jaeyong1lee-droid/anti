const qText = `다음 표는 동일한 점토 광물로 구성된 두 지반 X 와 Y 의 간극수 화학적 특성을 나타낸 것이다. Gouy – Chapman 이론에 근거하여 이중층 두께 t 와 지반공학적 특성 변화를 올바르게 설명한 것은 무엇인가? < tableborder = "1"style = "border – collapse: collapse; width: 100%; text – align: center;">< tr >< thstyle = "padding: 8px;">구분 < /th >< thstyle = "padding: 8px;">점토 지반 X < /th >< thstyle = "padding: 8px;">점토 지반 Y < /th >< /tr >< tr >< tdstyle = "padding: 8px;">퇴적 환경 < /td >< tdstyle = "padding: 8px;">해수 환경 (고농도 전해질 ) < /td >< tdstyle = "padding: 8px;">담수 환경 (저농도 전해질 ) < /td >< /tr >< tr >< tdstyle = "padding: 8px;">주요 양이온 < /td >< tdstyle = "padding: 8px;">다가 이온 (Ca2+ ) < /td >< tdstyle = "padding: 8px;">단가 이온 (Na+ ) < /td >< /tr >< /table >`;

function parseQuestionTable(q) {
  let questionText = q.question || '';
  let tableData = q.tableData || null;

  const hasTable = questionText.toLowerCase().includes('<table') || questionText.toLowerCase().replace(/\s+/g, '').includes('<table');
...
const res = parseQuestionTable({ question: qText });
console.log("=== FINAL ===");
console.log("questionText:", res.questionText);
console.log("tableData:", res.tableData);
