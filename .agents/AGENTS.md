...
8. **원인 규명 및 근본적 제거 우선 수칙 (Root Cause Removal First)**:
   - 뭔가 문제가 발생하거나 비정상적인 결과(예: 환각, 렌더링 깨짐 등)가 나타났을 때, **잘못된 결과를 억지로 보완(땜질)하는 방어용 코딩이나 정규식(Regex)을 추가하기 전에, 당초 그 문제를 유발한 '원인'(예: 잘못된 프롬프트 지침이나 기존의 과도한 코드)을 찾아내어 과감하게 '삭제'하거나 '개조'하는 방안을 먼저 최우선으로 검토하십시오.**
   - 방어용/우회용 코드는 시스템을 점점 무겁고 복잡하게(스파게티 코드) 만드므로, 원인을 근본적으로 제거하는 가장 단순하고 우아한 건축적 해결책을 지향해야 합니다.

9. **LATEX 렌더링 로직 방어쉴드 수칙 (LATEX Rendering Logic Defense Shield) [극도로 중요]**:
   - 시스템의 **메인 렌더링 파이프라인(`LatexRenderer.jsx`, `latexUtils.js` 등)**은 매우 민감하고 복잡한 코어 로직이므로, **어떠한 일이 있어도 사용자의 명시적인 사전 허락(승인) 없이 임의로 수정하거나 훼손해서는 안 됩니다.**
   - 특정 컴포넌트(예: 차트, 표 등) 내부에서만 필요한 특수한 KaTeX/LATEX 렌더링 처리가 있다면, **절대 메인 렌더링 함수를 뜯어고치지 말고, 필요한 기능만 가볍게 가져다(import) 쓰거나 해당 컴포넌트 내부에서만 작동하는 독립적인(격리된) 자체 헬퍼 함수를 만들어 해결**하십시오.
