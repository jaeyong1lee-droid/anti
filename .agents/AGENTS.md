...
4. **답변 생성 시 최종 렌더링 결과 및 테스트 결과 자동 시각화 표출 의무**:
   - UI 렌더링, 수식, SVG/TikZ/Mermaid 도해, CSS 레이아웃, 코드 수정 작업이 포함된 대화 답변을 작성할 때는 **예외 없이 답변의 가장 마지막 부분에 반드시 실제 렌더링된 최종 출력 화면 이미지(또는 터미널 TEST 실행 결과 화면)를 시각적으로 포함하여 시연/보고**해야 합니다.
   - 사용자가 별도로 독촉하거나 지시하지 않더라도 항시 자동으로 최종 렌더링 결과물이 표출되도록 철저히 이행하십시오.
