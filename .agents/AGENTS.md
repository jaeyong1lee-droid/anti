...
## 자가 개선 및 검증 철칙 (Self-Improvement Testing Directive)

4. **모든 지시 사항 자가 개선 테스터 검증 의무화**:
   - 사용자가 요청하는 모든 코드 수정, UI 조정, 기능 추가/수정 지시사항에 대해 작업 수행 직후 **반드시 자가 개선 테스터(빌드 컴파일 검증, 레이아웃/동작 무결성 테스트)가 직접 가동되어 100% 무오류 수치 결과를 검증**해야 합니다.
   - 답변 출력 시 검증 보고서(`[🤖 자가 개선 테스터 검증 보고서]`) 항목을 항상 명시하여 테스터 검증 내역을 투명하게 공유하십시오.
