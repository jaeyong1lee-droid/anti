...
5. **자가 개선 테스터 사전 3단계 교정 검증 수칙 (Pre-Commit 3-Step Verification)**:
   ...
   - ② **프론트엔드 UI/구문 검증**: `App.jsx` 등 프론트엔드 파일 수정 시, JSX 찌꺼기 문자열(`{{ ... }}`)이나 미반영 변수가 없는지 렌더링부 전수 조사 및 구문/빌드 오류 사전 검사 수행.
   - ③ **서버/세션/배포 무결성 검증**: DB/세션 API 응답 데이터 구조와 React UI 상태가 상호 완전 일치하는지 확인하고, Vercel 배포 실패 리스크를 사전 차단한 후 커밋/푸시 수행.
